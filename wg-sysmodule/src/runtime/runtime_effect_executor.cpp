...
NOINLINE void RuntimeEffectExecutor::ExecuteOpenUdpBind(const OpenUdpBindEffect& effect, EffectBatch& generated) {
    ...
    logger::Log("UDP bind open completion peer=%u activation=%u socket_generation=%u socket=%d endpoint=%s error=%u",
                effect.peer.peer_index.Value(), effect.peer.activation_generation.Value(), effect.socket_generation.Value(),
                static_cast<int>(socket), effect.endpoint_text.data(), static_cast<unsigned int>(error));
...
NOINLINE void RuntimeEffectExecutor::ExecutePendingDatagramSend(const SendPendingDatagramEffect& effect, EffectBatch& generated) {
    ...
    if (!snapshot_available) {
        logger::Log("Abandoned encrypted datagram send peer=%u activation=%u datagram_generation=%u reason=snapshot_rejected "
                    ...
                    binding.IsOpen() ? 1U : 0U, binding.suspended ? 1U : 0U, binding.generation.Value(), static_cast<int>(binding.socket));
        ...
        std::visit(
            [this, &generated](const auto& value) {
                ...
                if constexpr (std::is_same_v<Effect, ResolveEndpointEffect>) {
                    ...
                    if (current) {
                        logger::Log("Queued endpoint resolution for peer %u activation=%u endpoint='%s' schedule=%u",
                                    value.peer.peer_index.Value(), value.peer.activation_generation.Value(), value.endpoint.data(),
                                    schedule ? 1U : 0U);
                    }
                    if (schedule) {
                    ...
                    {
                        std::scoped_lock lock(m_state_mutex);
                        completion = m_coordinator.Dispatch(NetworkPathRequestStartedEvent{
                            .peer = value.peer,
...
bool RuntimeEffectExecutor::PublishDecryptedPacketLocked(const PeerIdentity& peer, std::span<const std::uint8_t> inner_packet) {
    ...
    if (probe.validation == wgnx::wireguard::DebugProbeReplyValidation::Valid) {
        ...
        logger::Log("Validated debug ICMP reply for peer %u action=%s source=%s destination=%s seq=%u activation=%u",
                    peer.peer_index.Value(), wgnx::GetDebugTriggerActionName(probe.info.action), inner_source, inner_destination,
                    static_cast<unsigned int>(probe.info.sequence), probe.info.activation_generation);
        return true;
    }
    ...
    case PacketDeliveryStatus::Queued:
        logger::Log("Queued decrypted inner packet id=%llu peer=%u activation=%u bytes=%zu depth=%zu",
                    static_cast<unsigned long long>(delivery.packet_id.Value()), peer.peer_index.Value(),
                    peer.activation_generation.Value(), inner_packet.size(), delivery.queue_depth);
        break;
    case PacketDeliveryStatus::NoConsumer:
        logger::Log("Dropped decrypted inner packet peer=%u activation=%u bytes=%zu reason=no_consumer", peer.peer_index.Value(),
                    peer.activation_generation.Value(), inner_packet.size());
        break;
    case PacketDeliveryStatus::MalformedPacket:
        logger::Log("Dropped decrypted inner packet peer=%u activation=%u bytes=%zu validation=%s", peer.peer_index.Value(),
                    peer.activation_generation.Value(), inner_packet.size(),
                    wgnx::wireguard::GetInnerIpValidationErrorName(delivery.validation));
        break;
    case PacketDeliveryStatus::UnsupportedPacket:
        logger::Log("Dropped decrypted inner packet peer=%u activation=%u bytes=%zu reason=unsupported_transport_ip_version version=%u",
                    peer.peer_index.Value(), peer.activation_generation.Value(), inner_packet.size(),
                    static_cast<unsigned int>(delivery.version));
        break;
    case PacketDeliveryStatus::QueueFull:
        logger::Log("Dropped decrypted inner packet peer=%u activation=%u bytes=%zu reason=rx_queue_full capacity=%zu",
                    peer.peer_index.Value(), peer.activation_generation.Value(), inner_packet.size(), delivery.queue_capacity);
        break;
...
void RuntimeEffectExecutor::CommitDebugPayloadSubmission(const DebugProbeRequest& request) {
    ...
    if (!m_coordinator.IsActiveIdentity(request.peer)) {
        ...
        logger::Log("Discarded queued payload submission for stale peer %u activation=%u", request.peer.peer_index.Value(),
    ...
    if (!m_coordinator.IsActiveEstablishedIdentity(request.peer) || !binding.IsOpen()) {
        static_cast<void>(m_debug_probe_runner.MarkFailed(request,
                                                          ...
                                                          GetRuntimeNowNs()));
        logger::Log("Discarded queued payload submission for peer %u state=%s activation=%u current_activation=%u",
                    request.peer.peer_index.Value(), wgnx::GetPeerRuntimeStateName(lifecycle->state),
                    request.peer.activation_generation.Value(), lifecycle->activation_generation.Value());
        return;
    }
    ...
    if (payload_size == 0) {
        ...
        logger::Log("Failed to build debug ICMP packet for peer %u activation=%u action=%s source=%s target=%s",
                    request.peer.peer_index.Value(), request.peer.activation_generation.Value(),
                    wgnx::GetDebugTriggerActionName(request.action), request.source_address.data(), target_text);
        return;
    }
    ...
    wgnx::wireguard::CopyDebugTargetIpv4(request.action, target_ipv4);
    wgnx::wireguard::FormatIpv4Text(target_ipv4, target_text, sizeof(target_text));
    logger::Log("Built debug ICMP packet for peer %u activation=%u action=%s source=%s target=%s bytes=%zu",
                request.peer.peer_index.Value(), request.peer.activation_generation.Value(),
                wgnx::GetDebugTriggerActionName(request.action), request.source_address.data(), target_text, payload_size);
    const auto submission = m_packet_data_plane.SubmitInternalIpPacket(std::span<const std::uint8_t>(payload.data(), payload_size),
                                                                       timer_facts, GetRuntimeNowNs(), effects);
    if (submission.status != PacketSubmissionStatus::Queued) {
        ...
        logger::Log("Failed to stage debug payload peer=%u activation=%u status=%u", request.peer.peer_index.Value(),
                    request.peer.activation_generation.Value(), static_cast<unsigned int>(submission.status));
        return;
    }
...
void RuntimeEffectExecutor::RunEndpointResolver() {
    ...
    while (true) {
        ...
        logger::Log("Endpoint resolution completion peer=%u activation=%u endpoint='%s' success=%u stage=%s code=%s resolved=%s",
                    request->peer.peer_index.Value(), request->peer.activation_generation.Value(), request->endpoint.data(),
                    result.success ? 1U : 0U, wgnx::GetPeerErrorStageName(result.error_stage),
                    wgnx::GetPeerErrorCodeName(result.error_code), result.text.data());
        EffectBatch effects{};
        {
            std::scoped_lock lock(m_state_mutex);
...
void RuntimeEffectExecutor::RunProtocolTimer(wgnx::wireguard::TimerHook hook, const wgnx::wireguard::TimerToken& token) {
    ...
    const EffectBatch effects = m_coordinator.Dispatch(ProtocolTimerExpiredEvent{
        .peer =
            {
...
void RuntimeEffectExecutor::RunDebugProbeTimeout() {
    ...
    logger::Log("Debug payload probe timed out for peer %u action=%s activation=%u", peer.peer_index.Value(),
                wgnx::GetDebugTriggerActionName(action), peer.activation_generation.Value());
}
