...
void PeerRuntime::UpdateEndpointFromAuthenticatedPacket(const EncryptedDatagramReceivedEvent& event) {
...
void PeerRuntime::HandleTransportData(const EncryptedDatagramReceivedEvent& event, EffectBatch& effects) {
    ...
    if (result.promoted_next_keypair) {
        ...
        if (m_lifecycle.state == wgnx::PeerRuntimeState::Handshaking) {
            static_cast<void>(EnterActive(event.peer.activation_generation, event.occurred_at));
        } else {
            m_lifecycle.last_handshake_ns = event.occurred_at;
            m_lifecycle.established = true;
        }
    ...
    const auto padded_payload = std::span<const std::uint8_t>(m_decrypted_packet.bytes.data(), result.decrypt.payload_size);
...
void PeerRuntime::HandleEncryptedDatagram(const EncryptedDatagramReceivedEvent& event, EffectBatch& effects) {
    auto* peer = ProtocolPeer();
    const auto packet = event.packet.Bytes();
    if (!IsCurrentActivation(event.peer.activation_generation) || !IsInTransportState() || peer == nullptr || packet.empty()) {
        return;
    }
