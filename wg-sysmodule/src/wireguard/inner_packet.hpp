...
constexpr inline std::size_t MaxInnerIpv4PacketSize = wgnx::resource_budget::MaximumInnerPacketBytes;
...
static_assert(sizeof(InnerPacketRecord) <= wgnx::resource_budget::MaximumInnerPacketRecordBytes);
...
struct QueueStatistics {
    ...
    std::uint64_t Dropped() const {
        return rejected_full + stale + unavailable + send_failed + retry_exhausted + cleared;
    }
};

template <std::size_t Capacity> class InnerPacketQueue {
  public:
  ...
  private:
    void RecordDisposition(QueueDisposition disposition, std::size_t count) {
        ...
        case QueueDisposition::Delivered:
            m_statistics.delivered += value;
            break;
        case QueueDisposition::Sent:
            m_statistics.sent += value;
            break;
        case QueueDisposition::Stale:
            m_statistics.stale += value;
            break;
        case QueueDisposition::Unavailable:
            m_statistics.unavailable += value;
            break;
        case QueueDisposition::SendFailed:
            m_statistics.send_failed += value;
            break;
        case QueueDisposition::RetryExhausted:
            m_statistics.retry_exhausted += value;
            break;
        case QueueDisposition::Cleared:
            m_statistics.cleared += value;
            break;
        }
    }
