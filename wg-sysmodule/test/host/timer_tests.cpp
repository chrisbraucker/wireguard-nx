...
void TestTimerIntent(TestContext& context) {
    ...
    wgnx::wireguard::wg_timers_schedule(&timers, wgnx::wireguard::TimerHook::RetransmitHandshake,
                                        wgnx::wireguard::TimerDeadlineFromJiffies(5'000), pair.initiator->name);
    wgnx::wireguard::wg_timers_schedule(&timers, wgnx::wireguard::TimerHook::NewHandshake,
                                        wgnx::wireguard::TimerDeadlineFromJiffies(120'000), pair.initiator->name);
    wgnx::wireguard::wg_timers_schedule(&timers, wgnx::wireguard::TimerHook::PersistentKeepalive,
                                        wgnx::wireguard::TimerDeadlineFromJiffies(25'000), pair.initiator->name);
    WGNX_TEST_REQUIRE(
        context,
    ...
    wgnx::wireguard::wg_timers_schedule(&timers, wgnx::wireguard::TimerHook::RetransmitHandshake,
                                        wgnx::wireguard::TimerDeadlineFromJiffies(7'500), pair.initiator->name);
    WGNX_TEST_REQUIRE(context,
                      timers.retransmit_handshake.pending &&
                          timers.retransmit_handshake.deadline == wgnx::wireguard::TimerDeadlineFromJiffies(7'500),
...
void TestTimerCoordinator(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context, first.IsValid() && coordinator.IsCurrent(first, first_owner),
    ...
    WGNX_TEST_REQUIRE(context, !coordinator.IsCurrent(replacement, first_owner) && !coordinator.IsArmed(TimerHook::ZeroKeyMaterial),
...
void TestTimerSchedule(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context, !schedule.Arm({}, TimerDeadlineFromJiffies(400)) && !schedule.CaptureExpiration(TimerHook::SendKeepalive),
