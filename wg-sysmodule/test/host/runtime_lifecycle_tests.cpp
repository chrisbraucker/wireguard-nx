...
void TestRuntimeResourceBudgets(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        ...
        wgnx::resource_budget::PeerSlots == wgnx::MaxPeers && wgnx::resource_budget::ActivePeerSlots == 1 &&
            ...
            wgnx::resource_budget::EffectBatchSlots == EffectBatch::Capacity && wgnx::resource_budget::MainThreadStackBytes == 16 * 1024 &&
            pressured.depth == 1 && pressured.high_watermark == 1 && pressured.admitted == 2 && pressured.replaced == 1 &&
            pressured.coalesced == 0 && pressured.taken == 0 && final.depth == 0 && final.admitted == 3 && final.taken == 1 &&
            final.cancelled == 1,
...
void TestIpcServerShutdownLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        lifecycle.Phase() == IpcServerLifecyclePhase::Idle && !lifecycle.BeginStopping() && !lifecycle.MarkServerThreadJoined() &&
            !lifecycle.MarkProcessExitReady() && lifecycle.BeginServing() && !lifecycle.BeginServing() && lifecycle.BeginStopping() &&
            !lifecycle.MarkProcessExitReady() && lifecycle.MarkServerThreadJoined() && lifecycle.MarkProcessExitReady() &&
...
void TestRuntimeTypedRejections(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        resolve != nullptr && pre_protocol_submission.status == PacketSubmissionStatus::InternalError,
    ...
    WGNX_TEST_REQUIRE(
        context,
        !activated.Empty() && data_plane.DeliverDecryptedPacket(identity, ValidPacket).status == PacketDeliveryStatus::NoConsumer,
    ...
    WGNX_TEST_REQUIRE(
        context,
        malformed_delivery.status == PacketDeliveryStatus::MalformedPacket && stale_delivery.status == PacketDeliveryStatus::StalePeer &&
...
void TestAuxiliaryRuntimeWorkflows(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        unsupported_probe == DebugProbeQueueResult::UnsupportedAction && invalid_source_probe == DebugProbeQueueResult::InvalidSource &&
            valid_probe == DebugProbeQueueResult::Queued && busy_probe == DebugProbeQueueResult::Busy && probes.IsPending() &&
    ...
    WGNX_TEST_REQUIRE(
        ...
        first_rebind == UdpRebindQueueResult::Scheduled && replacement_rebind == UdpRebindQueueResult::Replaced &&
            ...
            rebind_statistics.high_watermark == 1 && pending_before_take && taken.has_value() && consumed_rebind_statistics.depth == 0 &&
            consumed_rebind_statistics.taken == 1 && taken->peer_index == rebind.peer_index &&
            taken->activation_generation == rebind.activation_generation && !rebinds.Take().has_value(),
...
void TestRuntimeContracts(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        IsValidPeerSelection(-1, 0) && !IsValidPeerSelection(0, 0) && !IsValidPeerSelection(-2, 8) && IsValidPeerSelection(0, 8) &&
            IsValidPeerSelection(7, 8) && !IsValidPeerSelection(8, 8),
    ...
    WGNX_TEST_REQUIRE(
        ...
        BuildDaemonFlags(false, false) == wgnx::DaemonFlag_Ready &&
            ...
            BuildDaemonFlags(true, true) == (wgnx::DaemonFlag_Ready | wgnx::DaemonFlag_TunnelActive | wgnx::DaemonFlag_HasErrors),
    ...
    WGNX_TEST_REQUIRE(
        ...
        MaxEffectsForEvent<ActivationRequestedEvent>() == 5 && MaxEffectsForEvent<DeactivationRequestedEvent>() == 7 &&
            MaxEffectsForEvent<NetworkPathRequestStartedEvent>() == 5 && MaxEffectsForEvent<NetworkPathAvailabilityChangedEvent>() == 7 &&
            ...
            MaxEffectsForEvent<EncryptedDatagramReceivedEvent>() == 6 && MaxEffectsForEvent<PendingDatagramSentEvent>() == 5 &&
            MaxEffectsForEvent<InnerPacketStagedEvent>() == 5 && MaxEffectsForEvent<ProcessOutboundQueueEvent>() == 5 &&
...
void TestNifmPathGating(TestContext& context) {
    ...
    const auto pending = coordinator.Dispatch(
    ...
    WGNX_TEST_REQUIRE(
        context,
        resolve != nullptr && !handshake.Empty() && coordinator.BindingSnapshot(0).IsOpen(),
    ...
    const auto indeterminate = coordinator.Dispatch(
        NetworkPathAvailabilityChangedEvent{
            .peer = start->peer,
            .path_generation = start->path_generation,
            .observation =
    ...
    WGNX_TEST_REQUIRE(
        context,
        !confirmed_rebind_completion.Empty() && coordinator.BindingSnapshot(0).Matches(confirmed_rebind->socket_generation, 78),
        "confirmed indeterminate-path rebind did not adopt its replacement binding"
    );

    const auto suspended = coordinator.Dispatch(
        NetworkPathAvailabilityChangedEvent{
            .peer = start->peer,
            .path_generation = start->path_generation,
            .observation =
    ...
    const auto blocked_rebind = coordinator.Dispatch(
    ...
    WGNX_TEST_REQUIRE(
        context,
        !suspended.Empty() && binding_after_loss.suspended && !binding_after_loss.IsOpen() && blocked_rebind.Empty() && rebind != nullptr &&
            rebind->purpose == UdpBindPurpose::Rebind && rebind->replaces_socket == wgnx::platform::InvalidSocket,
        "OnHold local-path loss did not suspend once and resume through rebind"
    );

    const auto pending_rebind_completion = rebind != nullptr ? coordinator.Dispatch(
                                                                   UdpBindOpenedEvent{
                                                                       .peer = rebind->peer,
    ...
    const auto pending_blocked_rebind = coordinator.Dispatch(
        UdpRebindRequestedEvent{
            .peer = start->peer,
            .occurred_at = 1'013,
        }
    ...
    const auto close_count = std::ranges::count_if(pending_after_available, [](const auto& effect) {
        return std::holds_alternative<CloseUdpSocketEffect>(effect);
    });
    WGNX_TEST_REQUIRE(
        context,
        !pending_rebind_completion.Empty() && binding_after_pending.suspended && !binding_after_pending.IsOpen() && close_count == 1 &&
            pending_blocked_rebind.Empty() && pending_rebind != nullptr && pending_rebind->purpose == UdpBindPurpose::Rebind,
...
void TestPeerRegistryOwnership(TestContext& context) {
    ...
    const auto first_activation = coordinator.Dispatch(
        ActivationRequestedEvent{
            .peer_index = PeerIndex{0},
            .occurred_at = 6,
        }
...
void TestPeerRuntimeLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        ConfigureTestPeers(coordinator, configured, 0, -1, 1'000) && coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Inactive &&
    ...
    const auto failure = coordinator.Dispatch(
    ...
    const auto fatal = coordinator.Dispatch(
        TransportFailureEvent{
    ...
    static_cast<void>(coordinator.Dispatch(
    ...
    WGNX_TEST_REQUIRE(
        ...
        coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Inactive &&
            coordinator.Lifecycle(0)->activation_generation == ActivationGeneration{} && coordinator.Lifecycle(0)->rx_bytes == 0 &&
...
void TestRuntimeCoordinatorDispatch(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        ConfigureTestPeers(coordinator, configured, 0) && !ActivateTestPeer(coordinator, 0, 43).Empty(),
        "coordinator test could not establish handshaking state"
    );

    const auto out_of_range = coordinator.Dispatch(
    ...
    const auto effects = coordinator.Dispatch(
        EncryptedDatagramReceivedEvent{
            .peer = {.peer_index = PeerIndex{0}, .activation_generation = ActivationGeneration{1}},
            .occurred_at = 4'000,
        }
    ...
    WGNX_TEST_REQUIRE(
        context,
        effects.Empty() && coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Handshaking,
        "coordinator accepted session completion without protocol state"
    );

    const auto binding = coordinator.BindingSnapshot(0);
    const auto stale_failure = coordinator.Dispatch(
        TransportFailureEvent{
            .peer = {.peer_index = PeerIndex{0}, .activation_generation = ActivationGeneration{2}},
            .socket = binding.socket,
            .socket_generation = binding.generation,
    ...
    WGNX_TEST_REQUIRE(
        context,
        filled && bounded.Size() == EffectBatch::Capacity && bounded.TryAdd(effect) == EffectBatch::InsertionResult::CapacityExhausted,
    ...
    WGNX_TEST_REQUIRE(
        ...
        first.TryAdd(
            ...
            bounded.TryAppend(second) == EffectBatch::InsertionResult::CapacityExhausted && bounded.Size() == EffectBatch::Capacity,
        "runtime effect batch append lost ordering or violated bounded capacity"
    );
    bounded.Clear();
    WGNX_TEST_REQUIRE(
        context,
        bounded.Empty() && bounded.TryAdd(effect) == EffectBatch::InsertionResult::Inserted && bounded.Size() == 1,
...
void TestRuntimePeerActivation(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        ...
        coalesced_resolve == EndpointQueueResult::Coalesced && coalesced_resolve_statistics.admitted == 3 &&
            coalesced_resolve_statistics.replaced == 1 && coalesced_resolve_statistics.coalesced == 2 && coalesced_take.has_value() &&
            coalesced_take->peer == resolve->peer,
    ...
    const auto stale_resolution = coordinator.Dispatch(
    ...
    const auto opened = coordinator.Dispatch(
        UdpBindOpenedEvent{
            .peer = open->peer,
    ...
    WGNX_TEST_REQUIRE(
        ...
        opened.Size() == 1 && send != nullptr && coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Handshaking &&
            coordinator.ProtocolSnapshot(0).instantiated && coordinator.BindingSnapshot(0).Matches(open->socket_generation, 42),
    ...
    const auto sent = coordinator.Dispatch(
    ...
    WGNX_TEST_REQUIRE(
        ...
        sent.Size() == 4 && handshake_timer != nullptr && std::get_if<QueueReceiveEffect>(sent.begin() + 3) != nullptr &&
            ...
            coordinator.IsCurrentTimerEffect(*handshake_timer) && coordinator.Lifecycle(0)->tx_bytes == HandshakeInitiationSize &&
            !coordinator.SnapshotPendingDatagram(expected_activation, send->datagram_generation, snapshot),
    ...
    const auto invalid_effects = failed_coordinator.Dispatch(
        ActivationRequestedEvent{
            .peer_index = PeerIndex{0},
            .occurred_at = 5'000,
        }
...
void TestRuntimeOutboundLifecycle(TestContext& context) {
    ...
    const auto staged = coordinator.Dispatch(
    ...
    const auto stale_timer_effects = coordinator.Dispatch(
    ...
    WGNX_TEST_REQUIRE(
        context,
        stale_timer_effects.Empty() && coordinator.IsCurrentTimerEffect(*initial_arm),
    ...
    for (std::uint32_t attempt = 2; attempt <= MaxSendAttempts; ++attempt) {
        ...
        WGNX_TEST_REQUIRE(
            context,
            coordinator.SnapshotPendingDatagram(identity, send->datagram_generation, snapshot) && snapshot.size == HandshakeInitiationSize,
    ...
    WGNX_TEST_REQUIRE(
        context,
        effects.Size() == 3 && noise_handshake_begin_session(&responder.responder_device, responder.responder),
    ...
    WGNX_TEST_REQUIRE(
        context,
        effects.Size() == 5 && coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Active,
    ...
    WGNX_TEST_REQUIRE(
        ...
        coordinator.SnapshotPendingDatagram(identity, send->datagram_generation, snapshot) &&
            ...
            snapshot.binding.endpoint.port == roamed_endpoint.port && snapshot.binding.endpoint.address == roamed_endpoint.address,
    ...
    WGNX_TEST_REQUIRE(
        context,
        first_rebind_request.purpose == UdpBindPurpose::Rebind && binding_before_rebind.socket == 91,
    ...
    WGNX_TEST_REQUIRE(
        context,
        effects.Empty() && coordinator.BindingSnapshot(0).socket == 91 && coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Active,
    ...
    WGNX_TEST_REQUIRE(
        context,
        effects.Size() == 1 && std::get_if<CloseUdpSocketEffect>(effects.begin()) != nullptr && coordinator.BindingSnapshot(0).socket == 91,
    ...
    WGNX_TEST_REQUIRE(
        context,
        coordinator.SnapshotPendingDatagram(identity, rebind_send->datagram_generation, snapshot),
    ...
    const auto path_suspended = coordinator.Dispatch(
        NetworkPathAvailabilityChangedEvent{
            .peer = identity,
    ...
    effects = has_path_rebind ? coordinator.Dispatch(
                                    UdpBindOpenedEvent{
    ...
    WGNX_TEST_REQUIRE(
        context,
        has_path_rebind && send != nullptr && coordinator.SnapshotPendingDatagram(identity, recovered_send.datagram_generation, snapshot) &&
    ...
    WGNX_TEST_REQUIRE(
        context,
        !coordinator.HasPendingDatagram(identity, recovered_send.datagram_generation) && packet_state.staged_packet_count == 0,
    ...
    WGNX_TEST_REQUIRE(
        context,
        has_raced_send && coordinator.HasPendingDatagram(identity, raced_send_effect.datagram_generation) && !raced_snapshot_available &&
            packet_state.staged_packet_count == 1,
    ...
    WGNX_TEST_REQUIRE(
        context,
        !coordinator.HasPendingDatagram(identity, raced_send_effect.datagram_generation) && packet_state.staged_packet_count == 0 &&
            packet_state.can_stage_packet && coordinator.Lifecycle(0)->state == wgnx::PeerRuntimeState::Active,
...
void TestRuntimeInitiatorSessionKeepalive(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        initial_send != nullptr && coordinator.SnapshotPendingDatagram(identity, initial_send->datagram_generation, snapshot) &&
...
void TestAutoStartPersistenceGeneration(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        first.generation != replacement.generation && replacement.generation != clear.generation && !persistence.IsCurrent(first) &&
            !persistence.IsCurrent(replacement) && persistence.IsCurrent(clear) && clear.peer_index == -1 &&
            clear.peer_name.front() == '\0',
...
void TestUdpBindingOwnership(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        open.IsOpen() && open.Matches(SocketGeneration{3}, 44) && released == 44 && binding.IsSuspended() && suspended == 45 &&
