...
void TestFuzzedParsingBoundaries(TestContext& context) {
    const auto valid_config =
        wgnx::sysmodule::ValidateConnectionConfigLayout("[Interface]\nAddress = 10.13.13.8/24\n[Peer]\nEndpoint = 10.13.13.1:51820\n");
    WGNX_TEST_REQUIRE(context, valid_config.IsValid(), "valid config layout rejected");

    const auto duplicate_interface = wgnx::sysmodule::ValidateConnectionConfigLayout("[Interface]\n[Interface]\n[Peer]\n");
    ...
    WGNX_TEST_REQUIRE(context,
                      wgnx::platform::ParseEndpointText(" [2001:db8::1]:51820 ", &endpoint) &&
                          std::strcmp(endpoint.host.data(), "2001:db8::1") == 0 && std::strcmp(endpoint.service.data(), "51820") == 0 &&
                          wgnx::platform::ParseEndpointPort(endpoint.service.data()),
                      "bracketed IPv6 endpoint was not parsed");
    WGNX_TEST_REQUIRE(context,
                      !wgnx::platform::ParseEndpointText("2001:db8::1:51820", &endpoint) && !wgnx::platform::ParseEndpointPort("0") &&
...
void TestDeterministicHandshake(TestContext& context) {
    ...
    WGNX_TEST_CHECK(context, BuildDeterministicInitiation(&first, &first_state, &first_transitions, &first_transition_time));
    const runtime::State first_runtime = runtime::GetState();
    WGNX_TEST_CHECK(context, BuildDeterministicInitiation(&second, &second_state, &second_transitions, &second_transition_time));
    const runtime::State second_runtime = runtime::GetState();

    WGNX_TEST_REQUIRE(context, first == second, "same clock and random seed produced different initiation packets");
    WGNX_TEST_REQUIRE(context,
                      first_state == wgnx::wireguard::HandshakeState::InitiationCreated && second_state == first_state &&
                          first_transitions == 1 && second_transitions == first_transitions,
                      "initiation state transition was not deterministic");
    WGNX_TEST_REQUIRE(context,
                      first_transition_time ==
                              wgnx::wireguard::MonotonicTimePoint{wgnx::wireguard::MonotonicDuration{InitialMonotonicTime}} &&
...
void TestHandshakeInitiationAdmission(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context,
                      pair.CreateAndSendInitiation(&first_packet) && ParseHandshakeInitiation(first_packet, first).success &&
                          noise_handshake_consume_initiation(&first, pair.responder),
                      "first authenticated initiation was not admitted");
    WGNX_TEST_REQUIRE(context, !noise_handshake_consume_initiation(&first, pair.responder),
    ...
    runtime::SetMonotonicTime(InitialMonotonicTime + std::chrono::milliseconds{1}.count() * 1'000'000);
    ...
    runtime::SetMonotonicTime(InitialMonotonicTime + std::chrono::milliseconds{21}.count() * 1'000'000);
    WGNX_TEST_REQUIRE(context, noise_handshake_consume_initiation(&second, pair.responder),
    ...
    runtime::SetMonotonicTime(InitialMonotonicTime + std::chrono::milliseconds{42}.count() * 1'000'000);
...
void TestBidirectionalTransport(TestContext& context) {
    ...
    if (!CheckHandshakeState(context, *pair.initiator, wgnx::wireguard::HandshakeState::InitiationCreated, 1, "initiation sent")) {
        return;
    }
    WGNX_TEST_REQUIRE(context, pair.ReceiveInitiationAndSendResponse(), "initiation consumption or response serialization failed");
    if (!CheckHandshakeState(context, *pair.responder, wgnx::wireguard::HandshakeState::ResponseCreated, 2, "response sent")) {
        return;
    }
    WGNX_TEST_REQUIRE(context, pair.ReceiveResponseAndDeriveSession(), "response consumption or session derivation failed");
    if (!CheckHandshakeState(context, *pair.initiator, wgnx::wireguard::HandshakeState::SessionDerived, 3, "session established")) {
        return;
    }
    if (!CheckHandshakeState(context, *pair.responder, wgnx::wireguard::HandshakeState::SessionDerived, 3, "session established")) {
        return;
    }
    ...
    const TransportResult first = SendTransport(&pair.initiator->current_keypair, &pair.responder_device, pair.responder, InitiatorPayload,
                                                &pair.initiator_to_responder);
    WGNX_TEST_REQUIRE(context, first.error == wgnx::wireguard::TransportDataError::None,
                      wgnx::wireguard::GetTransportDataErrorName(first.error));
    WGNX_TEST_REQUIRE(context,
                      first.decrypt.header.counter == 0 && first.decrypt.payload_size == InitiatorPayload.size() &&
    ...
    const TransportResult reverse = SendTransport(&pair.responder->current_keypair, &pair.initiator_device, pair.initiator,
                                                  ResponderPayload, &pair.responder_to_initiator);
    WGNX_TEST_REQUIRE(context, reverse.error == wgnx::wireguard::TransportDataError::None,
                      wgnx::wireguard::GetTransportDataErrorName(reverse.error));
    WGNX_TEST_REQUIRE(context,
                      reverse.decrypt.header.counter == 0 && reverse.decrypt.payload_size == ResponderPayload.size() &&
    ...
    const TransportResult second = SendTransport(&pair.initiator->current_keypair, &pair.responder_device, pair.responder, InitiatorPayload,
                                                 &pair.initiator_to_responder);
    WGNX_TEST_REQUIRE(context, second.error == wgnx::wireguard::TransportDataError::None && second.decrypt.header.counter == 1,
    ...
    const auto replay_error =
        wgnx::wireguard::noise_consume_transport_data_packet(replay, pair.responder->current_keypair, replay_plaintext, replay_result);
    WGNX_TEST_REQUIRE(context, replay_error == wgnx::wireguard::TransportDataError::ReplayRejected,
                      wgnx::wireguard::GetTransportDataErrorName(replay_error));
    WGNX_TEST_REQUIRE(context,
                      pair.initiator->current_keypair.SendCounter() == 2 && pair.responder->current_keypair.SendCounter() == 1 &&
    ...
    WGNX_TEST_REQUIRE(context,
                      pair.initiator_to_responder.Pending() == 0 && pair.responder_to_initiator.Pending() == 0 &&
                          pair.initiator_to_responder.TotalSent() == 3 && pair.responder_to_initiator.TotalSent() == 2,
...
void TestFaultedDatagramLifecycle(TestContext& context) {
    ...
    ProtocolPair pair{};
    WGNX_TEST_REQUIRE(context,
                      pair.Initialize() && pair.CreateAndSendInitiation() && pair.ReceiveInitiationAndSendResponse() &&
    ...
    const auto first = noise_create_transport_data_packet(first_datagram, pair.initiator->current_keypair, FirstPayload);
    const auto second = noise_create_transport_data_packet(second_datagram, pair.initiator->current_keypair, SecondPayload);
    const auto third = noise_create_transport_data_packet(third_datagram, pair.initiator->current_keypair, ThirdPayload);
    WGNX_TEST_REQUIRE(context,
                      first.error == TransportDataError::None && first.counter == 0 && second.error == TransportDataError::None &&
                          second.counter == 1 && third.error == TransportDataError::None && third.counter == 2,
    ...
    const auto reordered =
        noise_consume_incoming_transport_data_packet(second_packet, pair.responder_device, *pair.responder, plaintext, decrypted);
    WGNX_TEST_REQUIRE(context,
                      reordered == TransportDataError::None && decrypted.promoted_next_keypair && decrypted.decrypt.header.counter == 1 &&
                          decrypted.decrypt.payload_size == SecondPayload.size() &&
                          std::ranges::equal(std::span<const std::uint8_t>(plaintext).first(decrypted.decrypt.payload_size), SecondPayload),
                      "out-of-order authenticated datagram was not accepted");

    const auto delayed =
        noise_consume_incoming_transport_data_packet(first_packet, pair.responder_device, *pair.responder, plaintext, decrypted);
    WGNX_TEST_REQUIRE(
        context,
        delayed == TransportDataError::None && !decrypted.promoted_next_keypair && decrypted.decrypt.header.counter == 0 &&
            std::ranges::equal(std::span<const std::uint8_t>(plaintext).first(decrypted.decrypt.payload_size), FirstPayload) &&
            pair.responder->current_keypair.ReceiveReplayWindow().HighestCounter() == 1,
    ...
    const auto authentication_failure =
        noise_consume_incoming_transport_data_packet(std::span<const std::uint8_t>(tampered_datagram).first(third.packet_size),
                                                     pair.responder_device, *pair.responder, plaintext, decrypted);
    const auto malformed = noise_consume_incoming_transport_data_packet(third_packet.first(TransportDataHeaderSize - 1),
                                                                        pair.responder_device, *pair.responder, plaintext, decrypted);
    WGNX_TEST_REQUIRE(context,
    ...
    const auto accepted =
        noise_consume_incoming_transport_data_packet(third_packet, pair.responder_device, *pair.responder, plaintext, decrypted);
    const auto accepted_counter = decrypted.decrypt.header.counter;
    const auto replay =
        noise_consume_incoming_transport_data_packet(third_packet, pair.responder_device, *pair.responder, plaintext, decrypted);
    WGNX_TEST_REQUIRE(context,
                      accepted == TransportDataError::None && accepted_counter == 2 && replay == TransportDataError::ReplayRejected &&
...
void TestKeyRotationDelayedDatagram(TestContext& context) {
    ...
    const TransportResult initial = SendTransport(&pair.initiator->current_keypair, &pair.responder_device, pair.responder, InitialPayload,
                                                  &pair.initiator_to_responder);
    WGNX_TEST_REQUIRE(context,
                      initial.error == TransportDataError::None && pair.responder->current_keypair.IsValid() &&
    ...
    const auto delayed_create = noise_create_transport_data_packet(delayed_datagram, pair.responder->current_keypair, DelayedPayload);
    WGNX_TEST_REQUIRE(context, delayed_create.error == TransportDataError::None && delayed_create.counter == 0,
                      "failed to construct the retained old-session datagram");
    const auto delayed_packet = std::span<const std::uint8_t>(delayed_datagram).first(delayed_create.packet_size);
    ...
    runtime::SetMonotonicTime(InitialMonotonicTime + std::chrono::seconds{1}.count() * 1'000'000'000LL);
    WGNX_TEST_REQUIRE(context,
                      pair.CreateAndSendInitiation() && pair.ReceiveInitiationAndSendResponse() && pair.ReceiveResponseAndDeriveSession(),
    ...
    WGNX_TEST_REQUIRE(
        ...
        pair.initiator->current_keypair.IsValid() && pair.responder->next_keypair.IsValid() &&
            std::ranges::equal(pair.initiator->current_keypair.SendingKey().bytes, pair.responder->next_keypair.ReceivingKey().bytes) &&
            std::ranges::equal(pair.initiator->current_keypair.ReceivingKey().bytes, pair.responder->next_keypair.SendingKey().bytes) &&
            pair.initiator->previous_keypair.IsValid() && pair.initiator->previous_keypair.LocalIndex() == old_receiver_index &&
            pair.responder->current_keypair.IsValid(),
        "replacement handshake did not retain and stage the expected keypairs");

    const TransportResult replacement = SendTransport(&pair.initiator->current_keypair, &pair.responder_device, pair.responder,
                                                      ReplacementPayload, &pair.initiator_to_responder);
    WGNX_TEST_REQUIRE(context,
                      replacement.error == TransportDataError::None && pair.responder->current_keypair.IsValid() &&
                          !pair.responder->next_keypair.IsValid() && pair.responder->previous_keypair.IsValid(),
                      "replacement transport did not rotate the responder keypairs");

    std::array<std::uint8_t, 64> plaintext{};
    IncomingTransportDataResult first_delivery{};
    const auto delayed =
        noise_consume_incoming_transport_data_packet(delayed_packet, pair.initiator_device, *pair.initiator, plaintext, first_delivery);
    WGNX_TEST_REQUIRE(
        context,
        delayed == TransportDataError::None && !first_delivery.promoted_next_keypair &&
            first_delivery.decrypt.header.receiver_index == old_receiver_index && first_delivery.decrypt.header.counter == 0 &&
            std::ranges::equal(std::span<const std::uint8_t>(plaintext).first(first_delivery.decrypt.payload_size), DelayedPayload),
        "delayed old-session datagram was not accepted through the previous keypair");

    IncomingTransportDataResult replay_delivery{};
    const auto replay =
        noise_consume_incoming_transport_data_packet(delayed_packet, pair.initiator_device, *pair.initiator, plaintext, replay_delivery);
    WGNX_TEST_REQUIRE(context,
...
void TestTypedMessageBoundaries(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context, SerializeHandshakeInitiation(initiation_bytes, initiation) == ParseError::None,
    ...
    for (std::size_t size = 0; size < HandshakeInitiationSize; ++size) {
        ...
        const ParseResult result = ParseHandshakeInitiation(std::span<const std::uint8_t>(initiation_bytes).first(size), parsed);
        if (result.success) {
    ...
    WGNX_TEST_REQUIRE(context, !oversized_result.success && oversized_result.error == ParseError::InvalidLength,
                      "fixed-size initiation parser accepted trailing bytes");

    std::array<std::uint8_t, HandshakeInitiationSize - 1> short_output{};
    WGNX_TEST_REQUIRE(context, SerializeHandshakeInitiation(short_output, initiation) == ParseError::InsufficientCapacity,
    ...
    WGNX_TEST_REQUIRE(context, SerializeTransportDataHeader(transport_bytes, transport) == ParseError::None,
    ...
    WGNX_TEST_REQUIRE(context,
                      ParseTransportDataHeader(transport_bytes, parsed_transport).success &&
                          parsed_transport.receiver_index == transport.receiver_index && parsed_transport.counter == transport.counter,
                      "transport parser rejected a checked variable-length packet");
    WGNX_TEST_REQUIRE(
        context,
        !ParseTransportDataHeader(std::span<const std::uint8_t>(transport_bytes).first(TransportDataHeaderSize - 1), parsed_transport)
             .success,
...
void TestPrivateKeyParsing(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context, noise_is_valid_encoded_key(ZeroKey), "all-zero key encoding was rejected as malformed base64");
    WGNX_TEST_REQUIRE(context, !noise_parse_private_key(&key, ZeroKey), "all-zero private key was accepted after X25519 clamping");
    WGNX_TEST_REQUIRE(context, key.valid && std::ranges::all_of(key.bytes, [](std::uint8_t byte) { return byte == 0xA5; }),
                      "failed private-key parsing modified the caller's key");

    WGNX_TEST_REQUIRE(context, noise_parse_private_key(&key, InitiatorPrivateKey), "valid private key was rejected");
    WGNX_TEST_REQUIRE(context,
                      key.valid && (key.bytes.front() & 0x07U) == 0 && (key.bytes.back() & 0x80U) == 0 && (key.bytes.back() & 0x40U) != 0,
...
void TestKeypairLifetime(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context,
                      !before_birth.valid && at_birth.valid && at_birth.age == MonotonicDuration::zero() && after_birth.valid &&
                          after_birth.age == std::chrono::seconds{3},
                      "keypair age evaluation depends on invalid or implicit clock state");

    keypair.Reset();
    WGNX_TEST_REQUIRE(context,
                      !keypair.IsValid() && keypair.State() == KeypairState::Empty && !keypair.SendingKey().valid &&
                          !keypair.ReceivingKey().valid &&
...
void TestBoundedQueueObservability(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context,
                      queue.Push(first) == QueuePushResult::Pushed && queue.Push(second) == QueuePushResult::Pushed &&
    ...
    WGNX_TEST_REQUIRE(context,
                      full_statistics.pushed == 2 && full_statistics.popped == 0 && full_statistics.rejected_full == 1 &&
                          full_statistics.Dropped() == 1 && full_statistics.high_watermark == 2,
    ...
    WGNX_TEST_REQUIRE(context,
                      queue.Pop(&popped, QueueDisposition::Delivered) && popped.packet_id == first.packet_id &&
                          queue.Statistics().popped == 1 && queue.Statistics().delivered == 1,
                      "bounded queue pop statistics diverged");

    WGNX_TEST_REQUIRE(context,
                      queue.Clear(QueueDisposition::Cleared) == 1 && queue.Size() == 0 &&
                          queue.Statistics().pushed == queue.Statistics().popped && queue.Statistics().cleared == 1,
...
void TestReplayWindowParity(TestContext& context) {
    ...
    ReplayWindow replay{};
    WGNX_TEST_REQUIRE(context, replay.TryAdvance(0) && !replay.TryAdvance(0), "replay filter did not reject a duplicate initial counter");

    constexpr std::uint64_t Highest = 10'000;
    constexpr std::uint64_t OldestAccepted = Highest - ReplayWindow::WindowSize;
    WGNX_TEST_REQUIRE(context,
                      replay.TryAdvance(Highest) && replay.TryAdvance(OldestAccepted) && !replay.TryAdvance(OldestAccepted) &&
                          !replay.TryAdvance(OldestAccepted - 1),
                      "replay filter diverged at the upstream 8,128-packet window boundary");

    replay.Reset();
    WGNX_TEST_REQUIRE(context, replay.TryAdvance(64) && replay.TryAdvance(0) && !replay.TryAdvance(64),
...
void TestPeerControllerSendPolicy(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context, peer.staged_outbound_packets.Push(record) == QueuePushResult::Pushed,
                      "failed to stage send-policy test packet");
    const auto sent = controller.ApplyStagedSendOutcome(peer, OutboundSendOutcome::Sent());
    WGNX_TEST_REQUIRE(context,
                      sent.retired && sent.disposition == QueueDisposition::Sent && sent.action == StagedPacketAction::Continue &&
    ...
    const auto expired = controller.ApplyStagedSendOutcome(peer, OutboundSendOutcome::BuildFailed(TransportDataError::KeyExpired));
    WGNX_TEST_REQUIRE(
        context, !expired.retired && expired.action == StagedPacketAction::InitiateHandshake && peer.staged_outbound_packets.Size() == 1,
        "peer controller did not retain plaintext for a replacement handshake");

    const auto dropped = controller.ApplyStagedSendOutcome(peer, OutboundSendOutcome::TransportDropped());
    WGNX_TEST_REQUIRE(context,
                      dropped.retired && dropped.disposition == QueueDisposition::SendFailed &&
                          dropped.action == StagedPacketAction::Continue && peer.staged_outbound_packets.Size() == 0,
                      "peer controller did not retire a packet lost by the UDP transport");

    record.packet_id = 3;
    static_cast<void>(peer.staged_outbound_packets.Push(record));
    const auto fatal = controller.ApplyStagedSendOutcome(peer, OutboundSendOutcome::BuildFailed(TransportDataError::AuthenticationFailed));
    WGNX_TEST_REQUIRE(
        context, !fatal.retired && fatal.action == StagedPacketAction::StopOnTerminalFailure && peer.staged_outbound_packets.Size() == 1,
...
void TestPeerControllerRecoveryWorkflow(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context,
                      transition.action == HandshakeTransitionAction::SendInitiation && pair.initiator->handshake_retry.send_attempts == 1,
    ...
    const auto observe_fresh_send = [&]() {
        ...
        const bool fresh =
            simulated_sends == 0 || (initiation.sender_index != previous_index && initiation.unencrypted_ephemeral != previous_ephemeral);
    ...
    for (std::uint32_t expected_attempt = 2; expected_attempt <= MaxSendAttempts; ++expected_attempt) {
        const TimerOwner owner{
            .peer_index = 0,
            .activation_generation = 1,
            .protocol_sequence = pair.initiator->handshake_retry.sequence_count,
        };
        const TimerToken token = controller.Timers().Arm(TimerHook::RetransmitHandshake, owner);
        WGNX_TEST_REQUIRE(context, controller.Timers().IsCurrent(token, owner), "controller retry timer was stale before delivery");
        controller.Timers().Cancel(TimerHook::RetransmitHandshake);

        transition = controller.HandleHandshakeRetryTimer(pair.initiator_device, *pair.initiator);
        WGNX_TEST_REQUIRE(context,
                          transition.action == HandshakeTransitionAction::SendInitiation &&
                              pair.initiator->handshake_retry.send_attempts == expected_attempt && observe_fresh_send(),
                          "controller retry did not construct and emit fresh initiation state");
    }

    transition = controller.HandleHandshakeRetryTimer(pair.initiator_device, *pair.initiator);
    WGNX_TEST_REQUIRE(context,
                      transition.action == HandshakeTransitionAction::Exhausted && transition.dropped_staged_packets == 1 &&
                          simulated_sends == MaxSendAttempts && pair.initiator->staged_outbound_packets.Size() == 0 &&
    ...
    later.bytes[2] = 0x00;
    later.bytes[3] = 0x14;
    WGNX_TEST_REQUIRE(context, pair.initiator->staged_outbound_packets.Push(later) == QueuePushResult::Pushed,
                      "failed to stage later traffic after exhaustion");
    transition = controller.StartHandshake(pair.initiator_device, *pair.initiator);
    WGNX_TEST_REQUIRE(context,
                      transition.action == HandshakeTransitionAction::SendInitiation &&
                          pair.initiator->handshake_retry.sequence_count == 2 && pair.initiator->handshake_retry.send_attempts == 1 &&
                          observe_fresh_send(),
                      "later traffic did not start a fresh controller sequence");

    std::array<std::uint8_t, HandshakeInitiationSize> initiation_packet{};
    WGNX_TEST_REQUIRE(context,
                      SerializeHandshakeInitiation(initiation_packet, pair.initiator->last_initiation) == ParseError::None &&
                          pair.initiator_to_responder.Send(initiation_packet) && pair.ReceiveInitiationAndSendResponse(),
    ...
    WGNX_TEST_REQUIRE(context,
                      pair.responder_to_initiator.Receive(response_packet) &&
                          noise_handshake_consume_incoming_packet(response_packet, &pair.initiator_device, pair.initiator) ==
    ...
    WGNX_TEST_REQUIRE(context,
                      controller.CompleteSession(pair.initiator_device, *pair.initiator) &&
                          ...
                          wg_peer_get_outbound_staging_action(*pair.initiator, GetMonotonicTime()) == OutboundStagingAction::Send,
    ...
    const TransportResult send =
        SendTransport(&pair.initiator->current_keypair, &pair.responder_device, pair.responder,
                      std::span<const std::uint8_t>(front->bytes.data(), front->size), &pair.initiator_to_responder);
    const auto sent = controller.ApplyStagedSendOutcome(*pair.initiator, send.error == TransportDataError::None
...
void TestKeypairProtocolLimits(TestContext& context) {
    ...
    keypair.Establish(InitiatorIndex, ResponderIndex, birth_time, sending_key, receiving_key);
    WGNX_TEST_REQUIRE(context,
                      keypair.SendStateAt(birth_time + RejectAfterTime - std::chrono::nanoseconds{1}) == KeypairSendState::Ready &&
    ...
    WGNX_TEST_REQUIRE(context,
                      !keypair.NeedsRekeyAt(birth_time + RekeyAfterTime) &&
                          keypair.NeedsRekeyAt(birth_time + RekeyAfterTime + std::chrono::nanoseconds{1}),
                      "initiator keypair did not request an age-based soft rekey at the upstream boundary");

    noise_keypair responder_keypair{};
    responder_keypair.Establish(InitiatorIndex, ResponderIndex, birth_time, sending_key, receiving_key, 0, false);
    WGNX_TEST_REQUIRE(context, !responder_keypair.NeedsRekeyAt(birth_time + RekeyAfterTime + std::chrono::nanoseconds{1}),
                      "responder keypair incorrectly initiated an age-based rekey");

    noise_keypair soft_counter_keypair{};
    soft_counter_keypair.Establish(InitiatorIndex, ResponderIndex, birth_time, sending_key, receiving_key, RekeyAfterMessages + 1);
    WGNX_TEST_REQUIRE(context, soft_counter_keypair.NeedsRekeyAt(birth_time),
                      "keypair did not request a message-count soft rekey above the upstream threshold");

    noise_keypair final_counter_keypair{};
    final_counter_keypair.Establish(InitiatorIndex, ResponderIndex, birth_time, sending_key, receiving_key, RejectAfterMessages - 1);
    std::uint64_t reserved_counter = 0;
    WGNX_TEST_REQUIRE(context,
                      final_counter_keypair.ReserveSendCounterAt(birth_time, reserved_counter) == KeypairSendState::Ready &&
                          reserved_counter == RejectAfterMessages - 1 && final_counter_keypair.SendCounter() == RejectAfterMessages &&
                          final_counter_keypair.ReserveSendCounterAt(birth_time, reserved_counter) == KeypairSendState::CounterExhausted,
                      "RejectAfterMessages boundary allowed an exhausted nonce");

    runtime::SetMonotonicTime(static_cast<wgnx::platform::ktime_t>((birth_time + RejectAfterTime).time_since_epoch().count()));
    std::array<std::uint8_t, 64> output{};
    std::ranges::fill(output, 0xA5);
    const auto expired_result = noise_create_transport_data_packet(output, keypair, std::span<const std::uint8_t>{});
    WGNX_TEST_REQUIRE(context,
                      expired_result.error == TransportDataError::KeyExpired && keypair.SendCounter() == 0 &&
                          std::ranges::all_of(output, [](std::uint8_t byte) { return byte == 0xA5; }),
                      "expired keypair emitted bytes or consumed a nonce");

    runtime::SetMonotonicTime(static_cast<wgnx::platform::ktime_t>(birth_time.time_since_epoch().count()));
    noise_keypair exhausted_keypair{};
    exhausted_keypair.Establish(InitiatorIndex, ResponderIndex, birth_time, sending_key, receiving_key, RejectAfterMessages);
    const auto exhausted_result = noise_create_transport_data_packet(output, exhausted_keypair, std::span<const std::uint8_t>{});
    WGNX_TEST_REQUIRE(
        context, exhausted_result.error == TransportDataError::CounterExhausted && exhausted_keypair.SendCounter() == RejectAfterMessages,
...
void TestOutboundStagingLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context, wg_peer_get_outbound_staging_action(*pair.initiator, GetMonotonicTime()) == OutboundStagingAction::Send,
    ...
    const TransportResult first_send =
        SendTransport(&pair.initiator->current_keypair, &pair.responder_device, pair.responder,
                      std::span<const std::uint8_t>(first_front->bytes.data(), first_front->size), &pair.initiator_to_responder);
    WGNX_TEST_REQUIRE(
        context,
        first_send.error == TransportDataError::None &&
            first_send.decrypt.payload_size == GetPaddedTransportPayloadSize(FirstPayload.size()) &&
            std::ranges::equal(FirstPayload, std::span<const std::uint8_t>(first_send.plaintext).first(FirstPayload.size())) &&
            std::ranges::all_of(std::span<const std::uint8_t>(first_send.plaintext)
                                    .subspan(FirstPayload.size(), first_send.decrypt.payload_size - FirstPayload.size()),
                                [](std::uint8_t byte) { return byte == 0; }),
        "staged packet was not transmitted after session derivation");
    InnerPacketRecord sent_record{};
    WGNX_TEST_REQUIRE(context,
                      pair.initiator->staged_outbound_packets.Pop(&sent_record, QueueDisposition::Sent) &&
                          sent_record.packet_id == first_record.packet_id,
                      "transmitted staged packet was not retired exactly once");

    const MonotonicTimePoint expired_time = pair.initiator->current_keypair.BirthTime() + RejectAfterTime;
    runtime::SetMonotonicTime(static_cast<wgnx::platform::ktime_t>(expired_time.time_since_epoch().count()));
    ...
    runtime::SetMonotonicTime(static_cast<wgnx::platform::ktime_t>(replacement_birth.time_since_epoch().count()));
    runtime::SetRealtime({
        .tv_sec = InitialRuntimeState.realtime.tv_sec + 1,
        .tv_nsec = InitialRuntimeState.realtime.tv_nsec,
    });
    ...
    WGNX_TEST_REQUIRE(context,
                      wg_peer_get_outbound_staging_action(*pair.initiator, GetMonotonicTime()) == OutboundStagingAction::Send &&
                          ...
                          pair.responder->current_keypair.LocalIndex() == old_responder_index && pair.responder->next_keypair.IsValid(),
    ...
    const TransportResult replacement_send = SendTransport(
        &pair.initiator->current_keypair, &pair.responder_device, pair.responder,
        std::span<const std::uint8_t>(replacement_front->bytes.data(), replacement_front->size), &pair.initiator_to_responder);
    WGNX_TEST_REQUIRE(context,
                      replacement_send.error == TransportDataError::None &&
                          ...
                          pair.responder->previous_keypair.LocalIndex() == old_responder_index && !pair.responder->next_keypair.IsValid(),
    ...
    for (std::size_t i = 0; i < PeerStagedPacketCapacity; ++i) {
        ...
        WGNX_TEST_REQUIRE(context, pair.initiator->staged_outbound_packets.Push(shutdown_record) == QueuePushResult::Pushed,
    ...
    WGNX_TEST_REQUIRE(context,
                      wg_peer_clear_staged_outbound_packets(pair.initiator) == PeerStagedPacketCapacity &&
                          ...
                          wg_peer_get_outbound_staging_action(*pair.initiator, GetMonotonicTime()) == OutboundStagingAction::Idle,
...
void TestHandshakeRetryLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(context,
                      wg_device_create_handshake_initiation(&pair.initiator_device, &first_initiation) &&
                          wg_device_create_handshake_initiation(&pair.initiator_device, &second_initiation),
    ...
    WGNX_TEST_REQUIRE(context,
                      first_initiation.sender_index != second_initiation.sender_index &&
                          ...
                          wg_device_lookup_index_slot(&pair.initiator_device, first_initiation.sender_index) == wg_index_slot::None &&
                          wg_device_lookup_index_slot(&pair.initiator_device, second_initiation.sender_index) == wg_index_slot::Handshake,
    ...
    WGNX_TEST_REQUIRE(context,
                      pair.initiator->handshake_retry.active && pair.initiator->handshake_retry.send_attempts == 1 &&
                          pair.initiator->handshake_retry.sequence_count == 1 && GetHandshakeRetryDelay(0) == std::chrono::seconds{5} &&
                          GetHandshakeRetryDelay(RekeyTimeoutJitterMaxMs - 1) == std::chrono::milliseconds{5333} &&
    ...
    for (std::uint32_t expected_attempt = 2; expected_attempt <= MaxSendAttempts; ++expected_attempt) {
        ...
        WGNX_TEST_REQUIRE(context,
                          retry.action == HandshakeRetryTimeoutAction::Retry && retry.dropped_staged_packets == 0 &&
    ...
    WGNX_TEST_REQUIRE(context,
                      exhausted.action == HandshakeRetryTimeoutAction::Exhausted && exhausted.dropped_staged_packets == 2 &&
                          !pair.initiator->handshake_retry.active && pair.initiator->handshake_retry.send_attempts == MaxSendAttempts &&
                          pair.initiator->handshake_retry.exhausted_sequence_count == 1 &&
                          pair.initiator->staged_outbound_packets.Size() == 0 && !pair.initiator->has_last_initiation,
                      "retry exhaustion retained failed work or left the sequence active");
    WGNX_TEST_REQUIRE(context, wg_peer_handle_handshake_retry_timeout(pair.initiator).action == HandshakeRetryTimeoutAction::Ignore,
    ...
    WGNX_TEST_REQUIRE(context,
                      !pair.initiator->handshake_retry.active && pair.initiator->handshake_retry.send_attempts == 0 &&
                          pair.initiator->staged_outbound_packets.Size() == 1,
    ...
    WGNX_TEST_REQUIRE(
        context,
        !pair.initiator->current_keypair.IsValid() && !pair.initiator->next_keypair.IsValid() &&
            !pair.initiator->previous_keypair.IsValid() && pair.initiator->handshake.state == HandshakeState::Zeroed &&
            pair.initiator->handshake.local_index == 0 && !pair.initiator->handshake_material.ephemeral_private.valid &&
            std::ranges::all_of(pair.initiator->handshake_material.ephemeral_private.bytes, [](std::uint8_t byte) { return byte == 0; }) &&
            !pair.initiator->handshake_material.hash.valid &&
            std::ranges::all_of(pair.initiator->handshake_material.hash.bytes, [](std::uint8_t byte) { return byte == 0; }),
    ...
    WGNX_TEST_REQUIRE(context,
                      pair.initiator->handshake_material.precomputed_static_static.valid &&
                          ...
                          pair.initiator->static_identity.static_private.bytes == static_private && pair.initiator->cookie.valid &&
                          std::ranges::all_of(pair.initiator->cookie.value, [](std::uint8_t byte) { return byte == 0x6A; }),
