...
void TestFuzzedParsingBoundaries(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        duplicate_interface.error == wgnx::sysmodule::ConfigLayoutError::MultipleInterfaceSections && duplicate_interface.line == 2,
    ...
    WGNX_TEST_REQUIRE(
        context,
        wgnx::platform::ParseEndpointText(" [2001:db8::1]:51820 ", &endpoint) && std::strcmp(endpoint.host.data(), "2001:db8::1") == 0 &&
            std::strcmp(endpoint.service.data(), "51820") == 0 && wgnx::platform::ParseEndpointPort(endpoint.service.data()),
...
void TestDeterministicHandshake(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        first_state == wgnx::wireguard::HandshakeState::InitiationCreated && second_state == first_state && first_transitions == 1 &&
            second_transitions == first_transitions,
        "initiation state transition was not deterministic"
    );
    WGNX_TEST_REQUIRE(
        context,
        first_transition_time == wgnx::wireguard::MonotonicTimePoint{wgnx::wireguard::MonotonicDuration{InitialMonotonicTime}} &&
...
void TestBidirectionalTransport(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        second.error == wgnx::wireguard::TransportDataError::None && second.decrypt.header.counter == 1,
...
void TestFaultedDatagramLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        first.error == TransportDataError::None && first.counter == 0 && second.error == TransportDataError::None && second.counter == 1 &&
    ...
    WGNX_TEST_REQUIRE(
        context,
        authentication_failure == TransportDataError::AuthenticationFailed && malformed == TransportDataError::InvalidPacket &&
            pair.responder->current_keypair.ReceiveReplayWindow().HighestCounter() == 1,
...
void TestKeyRotationDelayedDatagram(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        initial.error == TransportDataError::None && pair.responder->current_keypair.IsValid() && !pair.responder->next_keypair.IsValid(),
    ...
    WGNX_TEST_REQUIRE(
        context,
        replay == TransportDataError::ReplayRejected && pair.initiator->previous_keypair.ReceiveReplayWindow().HighestCounter() == 0,
...
void TestKeypairLifetime(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        !keypair.IsValid() && keypair.State() == KeypairState::Empty && !keypair.SendingKey().valid && !keypair.ReceivingKey().valid &&
...
void TestBoundedQueueObservability(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        queue.Pop(&popped, QueueDisposition::Delivered) && popped.packet_id == first.packet_id && queue.Statistics().popped == 1 &&
    ...
    WGNX_TEST_REQUIRE(
        context,
        queue.Clear(QueueDisposition::Cleared) == 1 && queue.Size() == 0 && queue.Statistics().pushed == queue.Statistics().popped &&
...
void TestPeerControllerSendPolicy(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        !expired.retired && expired.action == StagedPacketAction::InitiateHandshake && peer.staged_outbound_packets.Size() == 1,
    ...
    WGNX_TEST_REQUIRE(
        context,
        dropped.retired && dropped.disposition == QueueDisposition::SendFailed && dropped.action == StagedPacketAction::Continue &&
            peer.staged_outbound_packets.Size() == 0,
    ...
    WGNX_TEST_REQUIRE(
        context,
        !fatal.retired && fatal.action == StagedPacketAction::StopOnTerminalFailure && peer.staged_outbound_packets.Size() == 1,
...
void TestPeerControllerRecoveryWorkflow(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        ...
        pair.initiator->staged_outbound_packets.Push(first) == QueuePushResult::Pushed &&
            wg_peer_get_outbound_staging_action(*pair.initiator, GetMonotonicTime()) == OutboundStagingAction::InitiateHandshake,
    ...
    WGNX_TEST_REQUIRE(
        context,
        transition.action == HandshakeTransitionAction::SendInitiation && pair.initiator->handshake_retry.sequence_count == 2 &&
            pair.initiator->handshake_retry.send_attempts == 1 && observe_fresh_send(),
    ...
    WGNX_TEST_REQUIRE(
        ...
        controller.CompleteSession(pair.initiator_device, *pair.initiator) &&
            noise_handshake_begin_session(&pair.responder_device, pair.responder) && !pair.initiator->handshake_retry.active &&
    ...
    const auto sent = controller.ApplyStagedSendOutcome(
        *pair.initiator,
        send.error == TransportDataError::None ? OutboundSendOutcome::Sent() : OutboundSendOutcome::BuildFailed(send.error)
    );
    WGNX_TEST_REQUIRE(
        context,
...
void TestKeypairProtocolLimits(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        !responder_keypair.NeedsRekeyAt(birth_time + RekeyAfterTime + std::chrono::nanoseconds{1}),
    ...
    WGNX_TEST_REQUIRE(
        context,
        exhausted_result.error == TransportDataError::CounterExhausted && exhausted_keypair.SendCounter() == RejectAfterMessages,
...
void TestOutboundStagingLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        ...
        first_send.error == TransportDataError::None &&
            ...
            std::ranges::all_of(
                std::span<const std::uint8_t>(first_send.plaintext)
    ...
    WGNX_TEST_REQUIRE(
        context,
        wg_peer_get_outbound_staging_action(*pair.initiator, GetMonotonicTime()) == OutboundStagingAction::Send &&
            pair.initiator->staged_outbound_packets.Size() == 1 && pair.initiator->current_keypair.LocalIndex() != old_initiator_index &&
...
void TestHandshakeRetryLifecycle(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        ...
        exhausted.action == HandshakeRetryTimeoutAction::Exhausted && exhausted.dropped_staged_packets == 2 &&
            ...
            pair.initiator->handshake_retry.exhausted_sequence_count == 1 && pair.initiator->staged_outbound_packets.Size() == 0 &&
            !pair.initiator->has_last_initiation,
        "retry exhaustion retained failed work or left the sequence active"
    );
    WGNX_TEST_REQUIRE(
        context,
        wg_peer_handle_handshake_retry_timeout(pair.initiator).action == HandshakeRetryTimeoutAction::Ignore,
    ...
    WGNX_TEST_REQUIRE(
        context,
        pair.initiator->handshake_retry.active && pair.initiator->handshake_retry.send_attempts == 1 &&
            pair.initiator->handshake_retry.sequence_count == 2 && pair.initiator->handshake_retry.exhausted_sequence_count == 1 &&
            pair.initiator->staged_outbound_packets.Size() == 1,
