...
void TestPacketChannelOwnership(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        channel.PushReceived(received) == QueuePushResult::Pushed && channel.ReceivedSize() == 1,
...
void TestPacketDataPlane(TestContext& context) {
    ...
    WGNX_TEST_REQUIRE(
        context,
        ConfigureTestPeers(coordinator, configured, 0) && !ActivateTestPeer(coordinator, 0, 90).Empty(),
    ...
    WGNX_TEST_REQUIRE(
        ...
        unsupported.status == PacketDeliveryStatus::UnsupportedPacket && unsupported.version == InnerIpVersion::Ipv6 &&
            ...
            denied.status == PacketReceiveStatus::AccessDenied && too_small.status == PacketReceiveStatus::OutputBufferTooSmall &&
    ...
    for (std::size_t index = 0; index < PacketChannel::ReceiveCapacity; ++index) {
        WGNX_TEST_REQUIRE(
            context,
            data_plane.DeliverDecryptedPacket(identity, Ipv4Packet).status == PacketDeliveryStatus::Queued,
    ...
    WGNX_TEST_REQUIRE(
        context,
        transfer.status == PacketSubmissionStatus::Queued && transfer.ownership_transferred && transfer.discarded_outbound == 2 &&
            transfer.discarded_inbound == PacketChannel::ReceiveCapacity && channel.IsOwnedBy(SecondConsumer) &&
    ...
    WGNX_TEST_REQUIRE(
        context,
        stale_packet.status == PacketDeliveryStatus::Queued && coordinator.SetActivePeerIndex(-1),
        "packet data plane stale-delivery setup failed"
    );
    const auto stale = data_plane.ReceivePacket(output, SecondConsumer);
    WGNX_TEST_REQUIRE(
        context,
        stale.status == PacketReceiveStatus::StaleActivation && channel.Statistics().stale == 1 && channel.ReceivedSize() == 0,
