...
std::uint64_t NextRandom() {
    std::uint64_t value = g_random_state.fetch_add(0x9E3779B97F4A7C15ULL) + 0x9E3779B97F4A7C15ULL;
...
socket_error udp_send(socket_handle socket, const endpoint& destination, std::span<const std::uint8_t> data, std::size_t* out_sent) {
    if (socket == InvalidSocket || destination.family == address_family::unspecified || out_sent == nullptr) {
...
udp_receive_result udp_receive(socket_handle socket, std::span<std::uint8_t> buffer) {
    ...
    return classify_udp_receive_result(-1, udp_receive_native_condition::timed_out, 0);
}
