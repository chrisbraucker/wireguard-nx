...
[[nodiscard]] inline Result OpenTunnelClient(ScopedRootService& root, ScopedClient* out_client) {
    ...
    const Result rc = serviceDispatch(
        root.Get(),
        static_cast<std::uint32_t>(RootCommandId::OpenTunnelClient),
        .out_num_objects = 1,
        .out_objects = &child
    );
    if (R_SUCCEEDED(rc)) {
...
[[nodiscard]] inline Result GetRoutingPolicySnapshot(
    ...
    return serviceDispatchOut(
        client.Get(),
        static_cast<std::uint32_t>(ClientCommandId::GetRoutingPolicySnapshot),
        *out_snapshot,
...
[[nodiscard]] inline Result GetCompletionEvent(ScopedClient& client, Handle* out_handle) {
    ...
    *out_handle = INVALID_HANDLE;
    return serviceDispatch(
        client.Get(),
        static_cast<std::uint32_t>(ClientCommandId::GetCompletionEvent),
...
[[nodiscard]] inline Result SendUdpDatagramBatch(
    ...
    return serviceDispatch(
        client.Get(),
        static_cast<std::uint32_t>(ClientCommandId::SendUdpDatagramBatch),
        .buffer_attrs =
...
[[nodiscard]] inline Result ReceiveCompletions(
    ...
    const Result rc = serviceDispatchOut(
        client.Get(),
        static_cast<std::uint32_t>(ClientCommandId::ReceiveCompletions),
        result,
