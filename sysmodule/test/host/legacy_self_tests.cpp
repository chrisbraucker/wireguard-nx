...
// Internal self-test adapter defined in handshake.cpp, intentionally not part
// of the public handshake API.
...
ParseError SerializeLegacyPacket(wgnx::platform::packet_buffer* packet, const Message& message,
...
ParseError SerializeHandshakeInitiation(wgnx::platform::packet_buffer* packet, const message_handshake_initiation& message) {
    return SerializeLegacyPacket<HandshakeInitiationSize>(packet, message, ::wgnx::wireguard::SerializeHandshakeInitiation);
}

ParseError SerializeHandshakeResponse(wgnx::platform::packet_buffer* packet, const message_handshake_response& message) {
    return SerializeLegacyPacket<HandshakeResponseSize>(packet, message, ::wgnx::wireguard::SerializeHandshakeResponse);
}

ParseError SerializeHandshakeCookie(wgnx::platform::packet_buffer* packet, const message_handshake_cookie& message) {
    return SerializeLegacyPacket<HandshakeCookieSize>(packet, message, ::wgnx::wireguard::SerializeHandshakeCookie);
}

ParseError SerializeTransportDataHeader(wgnx::platform::packet_buffer* packet, const message_transport_data& message) {
    return SerializeLegacyPacket<TransportDataHeaderSize>(packet, message, ::wgnx::wireguard::SerializeTransportDataHeader);
}

template <typename Message>
ParseResult ParseLegacyPacket(const wgnx::platform::packet_buffer* packet, Message* message,
...
ParseResult ParseHandshakeInitiation(const wgnx::platform::packet_buffer* packet, message_handshake_initiation* message) {
    return ParseLegacyPacket(packet, message, ::wgnx::wireguard::ParseHandshakeInitiation);
}

ParseResult ParseHandshakeResponse(const wgnx::platform::packet_buffer* packet, message_handshake_response* message) {
    return ParseLegacyPacket(packet, message, ::wgnx::wireguard::ParseHandshakeResponse);
}

ParseResult ParseHandshakeCookie(const wgnx::platform::packet_buffer* packet, message_handshake_cookie* message) {
    return ParseLegacyPacket(packet, message, ::wgnx::wireguard::ParseHandshakeCookie);
}

ParseResult ParseTransportDataHeader(const wgnx::platform::packet_buffer* packet, message_transport_data* message) {
    return ParseLegacyPacket(packet, message, ::wgnx::wireguard::ParseTransportDataHeader);
}

ParseResult InspectMessageType(const wgnx::platform::packet_buffer* packet, MessageType* type) {
...
ParseResult DispatchPacket(const wgnx::platform::packet_buffer* packet, const PacketDispatchHandlers& handlers) {
    return packet == nullptr ? ParseResult{.success = false, .error = ParseError::InvalidArgument}
                             : ::wgnx::wireguard::DispatchPacket(packet->bytes(), handlers);
}

TransportDataError noise_create_transport_data_packet(wgnx::platform::packet_buffer* packet, noise_keypair& keypair,
                                                      std::span<const std::uint8_t> payload) {
    if (packet == nullptr) {
        return TransportDataError::InvalidArgument;
    }
    const TransportDataCreateResult result = ::wgnx::wireguard::noise_create_transport_data_packet(packet->storage(), keypair, payload);
...
bool noise_create_keepalive_packet(wgnx::platform::packet_buffer* packet, noise_keypair& keypair) {
    if (packet == nullptr) {
        return false;
    }
    const TransportDataCreateResult result = ::wgnx::wireguard::noise_create_keepalive_packet(packet->storage(), keypair);
    if (result.error == TransportDataError::None) {
        packet->len = result.packet_size;
    }
...
TransportDataError noise_consume_transport_data_packet(const wgnx::platform::packet_buffer* packet, noise_keypair* keypair,
                                                       std::span<std::uint8_t> output, TransportDataDecryptResult* result) {
    if (packet == nullptr || keypair == nullptr) {
        return TransportDataError::InvalidArgument;
    }
    TransportDataDecryptResult ignored{};
    return ::wgnx::wireguard::noise_consume_transport_data_packet(packet->bytes(), *keypair, output, result != nullptr ? *result : ignored);
}

TransportDataError noise_consume_incoming_transport_data_packet(const wgnx::platform::packet_buffer* packet, wg_device* device,
                                                                wg_peer* peer, std::span<std::uint8_t> output,
                                                                IncomingTransportDataResult* result) {
    if (packet == nullptr || device == nullptr || peer == nullptr) {
        return TransportDataError::InvalidArgument;
    }
    IncomingTransportDataResult ignored{};
    return ::wgnx::wireguard::noise_consume_incoming_transport_data_packet(packet->bytes(), *device, *peer, output,
                                                                           result != nullptr ? *result : ignored);
}

HandshakePacketOutcome noise_handshake_consume_incoming_packet(const wgnx::platform::packet_buffer* packet, wg_device* device,
                                                               wg_peer* peer) {
    if (packet == nullptr) {
        return HandshakePacketOutcome::Invalid;
    }
    return ::wgnx::wireguard::noise_handshake_consume_incoming_packet(packet->bytes(), device, peer);
}

void ResetCoreSelfTestStorage() {
    g_core_self_test_storage = {};
    wgnx::platform::packet_init(&g_core_self_test_storage.response_buffer.packet, g_core_self_test_storage.response_buffer.storage.data(),
                                g_core_self_test_storage.response_buffer.storage.size());
    wgnx::platform::packet_init(&g_core_self_test_storage.tampered_buffer.packet, g_core_self_test_storage.tampered_buffer.storage.data(),
                                g_core_self_test_storage.tampered_buffer.storage.size());
    wgnx::platform::packet_init(&g_core_self_test_storage.cookie_buffer.packet, g_core_self_test_storage.cookie_buffer.storage.data(),
                                g_core_self_test_storage.cookie_buffer.storage.size());
    wgnx::platform::packet_init(&g_core_self_test_storage.keepalive_buffer.packet, g_core_self_test_storage.keepalive_buffer.storage.data(),
                                g_core_self_test_storage.keepalive_buffer.storage.size());
    wgnx::platform::packet_init(&g_core_self_test_storage.payload_buffer.packet, g_core_self_test_storage.payload_buffer.storage.data(),
                                g_core_self_test_storage.payload_buffer.storage.size());
    wgnx::platform::packet_init(&g_core_self_test_storage.mismatch_buffer.packet, g_core_self_test_storage.mismatch_buffer.storage.data(),
                                g_core_self_test_storage.mismatch_buffer.storage.size());
    wgnx::platform::packet_init(&g_core_self_test_storage.tampered_payload_buffer.packet,
...
bool ComputeHarnessCookieKey(crypto::Blake2sDigest& out_key, const noise_public_key& remote_static) {
    ...
    return hasher.Initialize<crypto::Blake2sHashSize>() &&
           hasher.Update(crypto::ByteSpan{reinterpret_cast<const std::uint8_t*>(CookieKeyLabel), sizeof(CookieKeyLabel) - 1}) &&
           hasher.Update(remote_static.bytes) && hasher.Final(out_key);
}

bool BuildHarnessCookieReply(message_handshake_cookie* out_cookie, const wg_peer& initiator, const wg_peer& responder) {
    ...
    crypto::Blake2sDigest cookie_key{};
    ...
    *out_cookie = {};
    ...
    if (!crypto::xchacha20poly1305_encrypt(std::span{out_cookie->encrypted_cookie}.first(CookieValueSize), tag, CookieValue,
                                           initiator.cookie.last_mac1, cookie_key, out_cookie->nonce)) {
        crypto::secure_clear(cookie_key);
...
bool BuildHarnessDebugProbeReply(std::array<std::uint8_t, DebugProbePacketSize>* out_reply, wgnx::DebugTriggerAction action,
                                 std::uint32_t activation_generation, std::size_t peer_index) {
    ...
    *out_reply = {};
    const std::size_t request_size =
        BuildDebugIcmpEchoRequest(*out_reply, "10.13.13.2/32", action, activation_generation, peer_index, 0x11223344U);
    ...
    (*out_reply)[10] = 0;
    (*out_reply)[11] = 0;
    StoreHarnessBigEndian16(out_reply->data() + 10, ComputeHarnessInternetChecksum(out_reply->data(), DebugProbeIpv4HeaderSize));

    std::uint8_t* icmp = out_reply->data() + DebugProbeIpv4HeaderSize;
    icmp[0] = 0;
    icmp[2] = 0;
    icmp[3] = 0;
    StoreHarnessBigEndian16(icmp + 2, ComputeHarnessInternetChecksum(icmp, DebugProbeIcmpHeaderSize + DebugProbeIcmpPayloadSize));
    return true;
}
...
void CountTransportData(void* context, const message_transport_data&, std::span<const std::uint8_t> payload) {
...
template <typename Message>
bool RoundTripExact(const Message& source, ParseError (*serialize)(wgnx::platform::packet_buffer*, const Message&),
...
bool TestInnerIpv4PacketBoundary() {
    std::array<std::uint8_t, 20> packet = {
        0x45, 0x00, 0x00, 0x14, 0x12, 0x34, 0x00, 0x00, 0x40, 0x11, 0x00, 0x00, 10, 13, 13, 2, 10, 13, 13, 1,
    };
    StoreHarnessBigEndian16(packet.data() + 10, ComputeHarnessInternetChecksum(packet.data(), packet.size()));
    ...
    if (ValidatePaddedInnerIpv4Packet(packet, std::addressof(packet_size)) != InnerIpv4ValidationError::None ||
    ...
    if (ValidatePaddedInnerIpv4Packet(padded, std::addressof(packet_size)) != InnerIpv4ValidationError::None ||
        packet_size != packet.size()) {
        return false;
    }
    padded.back() = 1;
    if (ValidatePaddedInnerIpv4Packet(padded, std::addressof(packet_size)) != InnerIpv4ValidationError::InvalidPadding) {
        return false;
    }

    std::array<std::uint8_t, 36> excessive_padding{};
    std::memcpy(excessive_padding.data(), packet.data(), packet.size());
    if (ValidatePaddedInnerIpv4Packet(excessive_padding, std::addressof(packet_size)) != InnerIpv4ValidationError::InvalidPadding) {
        return false;
    }

    malformed = packet;
    malformed[3] = 0x15;
    if (ValidatePaddedInnerIpv4Packet(malformed, std::addressof(packet_size)) != InnerIpv4ValidationError::LengthMismatch) {
        return false;
    }
    malformed = packet;
    malformed[3] = 0x13;
    ...
    if (queue.Push(first) != QueuePushResult::Pushed || queue.Push(second) != QueuePushResult::Pushed ||
        queue.Push(overflow) != QueuePushResult::Full || queue.Size() != 2) {
        return false;
    }
...
bool TestPacketDispatch() {
    ...
    return result.success && context.transport_count == 1 && context.last_payload_size == sizeof(payload) &&
           context.initiation_count == 0 && context.response_count == 0 && context.cookie_count == 0;
}
...
bool TestTransportFixedVector() {
    ...
    const std::uint8_t expected[TransportDataHeaderSize] = {
        0x04, 0x00, 0x00, 0x00, 0xBE, 0xBA, 0xFE, 0xCA, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01,
    };
    ...
    message_transport_data parsed = {};
    const ParseResult result = ParseTransportDataHeader(&buffer.packet, &parsed);
    return result.success && GetMessageType(parsed.type) == MessageType::TransportData && parsed.receiver_index == message.receiver_index &&
...
bool TestDeviceAndPeerSkeleton() {
    ...
    if (peer->static_identity.static_private.valid || peer->handshake_material.chaining_key.valid) {
        return false;
    }
    ...
    static_cast<void>(
        noise_handshake_transition(&peer->handshake, HandshakeState::InitiationCreated, peer->name, "self-test create initiation"));
    static_cast<void>(
        noise_handshake_transition(&peer->handshake, HandshakeState::ResponseReceived, peer->name, "self-test receive response"));
    static_cast<void>(noise_handshake_transition(&peer->handshake, HandshakeState::SessionDerived, peer->name, "self-test derive session"));

    wg_timers_schedule(&peer->timers, TimerHook::RetransmitHandshake, TimerDeadlineFromJiffies(2000), peer->name);
    wg_timers_schedule(&peer->timers, TimerHook::NewHandshake, TimerDeadlineFromJiffies(4000), peer->name);
    ...
    return peer->handshake.state == HandshakeState::SessionDerived && peer->handshake.transition_count == 3;
}

bool TestStaticIdentityParsing() {
    noise_static_identity identity{};
    if (!noise_static_identity_init(&identity, HarnessLocalPrivateKey, HarnessRemotePublicKey, HarnessPresharedKey)) {
        return false;
    }
    ...
    const bool ok =
        identity.static_private.valid && identity.static_public.valid && identity.remote_static.valid && identity.preshared_key.valid &&
        std::memcmp(identity.static_public.bytes.data(), expected_local_public.bytes.data(), identity.static_public.bytes.size()) == 0;
...
bool TestHandshakeInitiationCreation() {
    ...
    return peer->has_last_initiation && peer->handshake.state == HandshakeState::InitiationCreated &&
           GetMessageType(parsed.type) == MessageType::HandshakeInitiation && parsed.sender_index == 0x01020304U &&
...
bool TestHandshakeResponseAndSessionDerivation() {
    ...
    if (noise_handshake_consume_incoming_packet(&tampered_buffer.packet, &initiator_device, initiator) != HandshakePacketOutcome::Invalid) {
        return false;
    }
    ...
    if (noise_handshake_consume_incoming_packet(&cookie_buffer.packet, &initiator_device, initiator) !=
    ...
    if (noise_handshake_consume_incoming_packet(&response_buffer.packet, &initiator_device, initiator) !=
        HandshakePacketOutcome::ResponseConsumed) {
        return false;
    }

    if (!noise_handshake_begin_session(&initiator_device, initiator) || !noise_handshake_begin_session(&responder_device, responder)) {
        return false;
    }
    ...
    const TransportDataError keepalive_error =
        noise_consume_incoming_transport_data_packet(&keepalive_buffer.packet, &responder_device, responder, {}, &keepalive_incoming);
    if (keepalive_error != TransportDataError::None) {
        return false;
    }
    const TransportDataDecryptResult& keepalive_result = keepalive_incoming.decrypt;
    if (keepalive_buffer.packet.len != (TransportDataHeaderSize + NoiseMacSize) || !keepalive_incoming.promoted_next_keypair ||
        keepalive_result.header.receiver_index != initiator->current_keypair.RemoteIndex() ||
        keepalive_result.header.counter + 1 != initiator->current_keypair.SendCounter() || keepalive_result.payload_size != 0) {
        return false;
    }
    ...
    if (noise_create_transport_data_packet(&payload_buffer.packet, initiator->current_keypair,
    ...
    responder_copy.current_keypair.Establish(responder->current_keypair.LocalIndex(), responder->current_keypair.RemoteIndex(),
                                             responder->current_keypair.BirthTime(), responder->current_keypair.SendingKey(),
                                             responder->current_keypair.ReceivingKey(), responder->current_keypair.SendCounter());

    TransportDataDecryptResult payload_result{};
    const TransportDataError payload_error = noise_consume_transport_data_packet(
        &payload_buffer.packet, &responder->current_keypair, g_core_self_test_storage.decrypted_payload, &payload_result);
    ...
    if (payload_result.header.receiver_index != initiator->current_keypair.RemoteIndex() ||
        ...
        std::memcmp(g_core_self_test_storage.payload_plaintext, g_core_self_test_storage.decrypted_payload,
                    sizeof(g_core_self_test_storage.payload_plaintext)) != 0) {
        return false;
    }

    const TransportDataError replay_error = noise_consume_transport_data_packet(&payload_buffer.packet, &responder->current_keypair,
                                                                                g_core_self_test_storage.decrypted_payload, nullptr);
    if (replay_error != TransportDataError::ReplayRejected) {
        return false;
    }

    auto& mismatch_buffer = g_core_self_test_storage.mismatch_buffer;
    std::memcpy(mismatch_buffer.packet.data, payload_buffer.packet.data, payload_buffer.packet.len);
    mismatch_buffer.packet.len = payload_buffer.packet.len;
    StoreLe32(mismatch_buffer.packet.data + sizeof(std::uint32_t), responder_copy.current_keypair.LocalIndex() ^ 0x00FF00FFU);
    const TransportDataError mismatch_error = noise_consume_transport_data_packet(&mismatch_buffer.packet, &responder_copy.current_keypair,
                                                                                  g_core_self_test_storage.decrypted_payload, nullptr);
    ...
    std::memcpy(tampered_payload_buffer.packet.data, payload_buffer.packet.data, payload_buffer.packet.len);
    ...
    const TransportDataError tampered_payload_error = noise_consume_transport_data_packet(
        &tampered_payload_buffer.packet, &responder_copy.current_keypair, g_core_self_test_storage.decrypted_payload, nullptr);
    if (tampered_payload_error != TransportDataError::AuthenticationFailed) {
        return false;
    }

    if (noise_create_transport_data_packet(&payload_buffer.packet, initiator->current_keypair,
                                           std::span<const std::uint8_t>(g_core_self_test_storage.payload_plaintext, 8)) !=
    ...
    const TransportDataError incoming_error = noise_consume_incoming_transport_data_packet(
        &payload_buffer.packet, &responder_device, &responder_copy, g_core_self_test_storage.decrypted_payload, &incoming_result);
    ...
    if (incoming_result.slot != wg_index_slot::CurrentKeypair ||
        ...
        std::memcmp(g_core_self_test_storage.payload_plaintext, g_core_self_test_storage.decrypted_payload, 8) != 0) {
        return false;
    }
    ...
    if (payload_buffer.packet.len != TransportDataHeaderSize + TransportDataPaddingBlockSize + NoiseTagSize) {
        return false;
    }

    return initiator->current_keypair.IsValid() && responder->current_keypair.IsValid() &&
           initiator->handshake.state == HandshakeState::SessionDerived && responder->handshake.state == HandshakeState::SessionDerived &&
           initiator->current_keypair.LocalIndex() == 0x01020304U && initiator->current_keypair.RemoteIndex() == 0xA1A2A3A4U &&
           responder->current_keypair.LocalIndex() == 0xA1A2A3A4U && responder->current_keypair.RemoteIndex() == 0x01020304U &&
           initiator->current_keypair.SendingKey().valid && initiator->current_keypair.ReceivingKey().valid &&
           responder->current_keypair.SendingKey().valid && responder->current_keypair.ReceivingKey().valid && initiator->cookie.valid &&
           initiator->has_last_initiation && responder->current_keypair.ReceiveReplayWindow().HasReceivedPacket() &&
           responder->current_keypair.ReceiveReplayWindow().HighestCounter() == payload_result.header.counter &&
           std::memcmp(initiator->current_keypair.SendingKey().bytes.data(), responder->current_keypair.ReceivingKey().bytes.data(),
                       initiator->current_keypair.SendingKey().bytes.size()) == 0 &&
           std::memcmp(initiator->current_keypair.ReceivingKey().bytes.data(), responder->current_keypair.SendingKey().bytes.data(),
                       initiator->current_keypair.ReceivingKey().bytes.size()) == 0 &&
           std::memcmp(initiator->last_initiation.macs.mac2.data(), ZeroMac, NoiseMacSize) != 0;
}
...
bool TestDebugProbeIcmpRoundTrip() {
    ...
    if (!BuildHarnessDebugProbeReply(std::addressof(reply), wgnx::DebugTriggerAction::PingTunnelPeer, 0x01020304U, 2)) {
        wgnx::sysmodule::logger::Log("Debug probe self-test: failed to build synthetic reply");
        return false;
    }

    DebugProbeReplyInfo info{};
    const DebugProbeReplyValidation validation = ValidateDebugIcmpEchoReply(reply, "10.13.13.2/32", 2, 0x01020304U, std::addressof(info));
    if (validation != DebugProbeReplyValidation::Valid) {
        wgnx::sysmodule::logger::Log("Debug probe self-test: unexpected validation=%s", GetDebugProbeReplyValidationName(validation));
        return false;
    }
    if (info.action != wgnx::DebugTriggerAction::PingTunnelPeer || info.activation_generation != 0x01020304U || info.peer_index != 2 ||
        ...
        wgnx::sysmodule::logger::Log(
            ...
            wgnx::GetDebugTriggerActionName(info.action), info.activation_generation, static_cast<unsigned int>(info.peer_index),
            static_cast<unsigned int>(info.source_ipv4[0]), static_cast<unsigned int>(info.source_ipv4[1]),
            static_cast<unsigned int>(info.source_ipv4[2]), static_cast<unsigned int>(info.source_ipv4[3]),
            static_cast<unsigned int>(info.destination_ipv4[0]), static_cast<unsigned int>(info.destination_ipv4[1]),
            static_cast<unsigned int>(info.destination_ipv4[2]), static_cast<unsigned int>(info.destination_ipv4[3]));
        return false;
    }
    ...
    reply[10] = 0;
    reply[11] = 0;
    StoreHarnessBigEndian16(reply.data() + 10, ComputeHarnessInternetChecksum(reply.data(), DebugProbeIpv4HeaderSize));
    const DebugProbeReplyValidation mismatch_validation = ValidateDebugIcmpEchoReply(reply, "10.13.13.2/32", 2, 0x01020304U, nullptr);
    if (mismatch_validation != DebugProbeReplyValidation::SourceMismatch) {
        wgnx::sysmodule::logger::Log("Debug probe self-test: expected source_mismatch got=%s",
...
bool RunMessageSelfTest() {
    const bool ok = TestHandshakeInitiation() && TestHandshakeResponse() && TestHandshakeCookie() && TestTransportDataHeader() &&
                    TestUnknownTypeRejected() && TestMalformedLengthRejected() && TestInnerIpv4PacketBoundary() && TestPacketDispatch() &&
