...
std::int32_t ElapsedSeconds(
    wgnx::platform::ktime_t timestamp,
    ...
    return elapsed > std::numeric_limits<std::int32_t>::max()
...
std::size_t DebugProbeRunner::BuildPacket(
...
DebugProbeReplyOutcome DebugProbeRunner::HandleDecryptedPacket(
    ...
    static_cast<void>(Transition(
        outcome.validation == wgnx::wireguard::DebugProbeReplyValidation::Valid
...
bool DebugProbeRunner::HandleTimeout(wgnx::platform::ktime_t now) {
    return m_status == wgnx::DebugProbeStatus::Sent &&
           Transition(wgnx::DebugProbeStatus::TimedOut, now);
}
...
bool DebugProbeRunner::IsPending() const {
    return m_status == wgnx::DebugProbeStatus::Queued ||
           m_status == wgnx::DebugProbeStatus::Sent;
}
...
bool DebugProbeRunner::Transition(
    wgnx::DebugProbeStatus status,
    wgnx::platform::ktime_t now) {
