...
void PeerRuntime::SuspendTransport(const PeerIdentity& identity, EffectBatch& effects) {
    if (m_binding.IsSuspended()) {
        logger::Log("UDP transport already suspended peer=%u activation=%u", identity.peer_index.Value(),
                    identity.activation_generation.Value());
        return;
    }

    const auto binding = m_binding.StateSnapshot();
    logger::Log("Suspending UDP transport for local-path unavailability peer=%u activation=%u socket_generation=%u socket=%d; preserving "
                "peer and protocol state",
                identity.peer_index.Value(), identity.activation_generation.Value(), binding.generation.Value(),
    ...
    logger::Log(
        ...
        identity.peer_index.Value(), identity.activation_generation.Value(), binding.generation.Value(), static_cast<int>(socket),
        wgnx::GetPeerRuntimeStateName(m_lifecycle.state));
}

void PeerRuntime::RecoverTransport(const PeerIdentity& identity, const TimerFacts& timer_facts, wgnx::platform::ktime_t now,
                                   EffectBatch& effects) {
    const auto binding = m_binding.StateSnapshot();
    if (m_pending_datagram.IsPending()) {
        logger::Log("UDP recovery deferred peer=%u activation=%u state=%s reason=pending_datagram kind=%s generation=%u binding_open=%u "
                    "binding_suspended=%u socket_generation=%u socket=%d",
                    identity.peer_index.Value(), identity.activation_generation.Value(), wgnx::GetPeerRuntimeStateName(m_lifecycle.state),
                    GetPendingDatagramKindName(m_pending_datagram.kind), m_pending_datagram.generation.Value(), binding.IsOpen() ? 1U : 0U,
                    binding.suspended ? 1U : 0U, binding.generation.Value(), static_cast<int>(binding.socket));
        return;
    }
    auto* peer = ProtocolPeer();
    if (peer == nullptr) {
        logger::Log("UDP recovery skipped peer=%u activation=%u state=%s reason=no_protocol_peer binding_open=%u binding_suspended=%u "
    ...
    if (m_lifecycle.state == wgnx::PeerRuntimeState::Active && current_key_can_send) {
        if (StagedInnerPacketCount() != 0) {
            logger::Log("UDP recovery draining staged inner packets peer=%u activation=%u count=%zu socket_generation=%u socket=%d",
                        identity.peer_index.Value(), identity.activation_generation.Value(), StagedInnerPacketCount(),
        ...
        logger::Log("UDP recovery selected keepalive peer=%u activation=%u socket_generation=%u socket=%d", identity.peer_index.Value(),
                    identity.activation_generation.Value(), binding.generation.Value(), static_cast<int>(binding.socket));
    ...
    logger::Log("UDP recovery selected handshake peer=%u activation=%u state=%s current_key_can_send=%u socket_generation=%u socket=%d",
                identity.peer_index.Value(), identity.activation_generation.Value(), wgnx::GetPeerRuntimeStateName(m_lifecycle.state),
                current_key_can_send ? 1U : 0U, binding.generation.Value(), static_cast<int>(binding.socket));
...
EffectBatch PeerRuntime::HandleEvent(const ProtocolTimerExpiredEvent& event) {
    ...
    if (!m_controller.Timers().IsCurrent(event.token, current_owner)) {
        logger::Log("Ignored stale WG timer event hook=%s token_peer=%u current_peer=%u token_activation=%u current_activation=%u "
                    "token_sequence=%u current_sequence=%u generation=%u",
                    wgnx::wireguard::GetTimerHookName(event.hook), event.token.owner.peer_index, event.peer.peer_index.Value(),
                    event.token.owner.activation_generation, event.peer.activation_generation.Value(), event.token.owner.protocol_sequence,
                    current_owner.protocol_sequence, event.token.generation);
        return effects;
    }
    logger::Log("WG timer peer='%s' fire hook=%s generation=%u", peer->name, wgnx::wireguard::GetTimerHookName(event.hook),
    ...
    if (m_binding.IsSuspended() && event.hook != wgnx::wireguard::TimerHook::ZeroKeyMaterial) {
        logger::Log("WG timer canceled for suspended UDP transport peer=%u activation=%u hook=%s", event.peer.peer_index.Value(),
                    event.peer.activation_generation.Value(), wgnx::wireguard::GetTimerHookName(event.hook));
        return effects;
    }
    ...
    case wgnx::wireguard::TimerHook::ZeroKeyMaterial:
        ...
        logger::Log("WG zeroed stale handshake and keypair material peer=%u activation=%u; peer and UDP binding preserved",
