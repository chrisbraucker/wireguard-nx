...
class RuntimeCoordinator {
  public:
    ...
    std::int32_t ActivePeerIndex() const {
        return m_peers.ActivePeerIndex();
    }
    std::int32_t AutoStartPeerIndex() const {
        return m_peers.AutoStartPeerIndex();
    }
    ...
    wgnx::PeerInfo BuildPeerInfo(std::size_t peer_index, wgnx::platform::ktime_t now) const;
    bool HasRuntimeErrors() const;
    bool IsActiveIdentity(const PeerIdentity& peer) const;
    bool IsActivePathIdentity(const PeerIdentity& peer, PathRequestGeneration path_generation) const;
    ...
    [[nodiscard]] bool SnapshotPendingDatagram(const PeerIdentity& peer, DatagramGeneration datagram_generation,
