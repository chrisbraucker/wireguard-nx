...
void ClearEncodedSecrets(wgnx::PeerConfigEntry &entry) {
    wgnx::wireguard::crypto::secure_clear(entry.private_key.data(), entry.private_key.size());
    wgnx::wireguard::crypto::secure_clear(entry.preshared_key.data(), entry.preshared_key.size());
}
...
PeerConfigDerivedInfo DeriveSecrets(wgnx::PeerConfigEntry &entry) {
    ...
    const bool preshared_key_valid = entry.preshared_key[0] == '\0' ||
...
bool PeerConfigurationLoader::Load(LoadedPeerConfiguration &out) const {
    ...
    const bool has_auto_start_name =
        LoadAutoStartPeerName(auto_start_name, sizeof(auto_start_name));
    ...
    for (std::size_t index = 0; index < out.peer_count; ++index) {
        ...
        if (has_auto_start_name &&
            std::strncmp(
                out.peers[index].name.data(),
                auto_start_name,
                out.peers[index].name.size()) == 0) {
