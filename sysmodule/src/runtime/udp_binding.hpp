...
class UdpRebindQueue {
  public:
    static constexpr std::size_t RequestCapacity = wgnx::resource_budget::UdpRebindRequestSlots;
    ...
    [[nodiscard]] UdpRebindQueueResult Queue(const UdpRebindRequest& request) {
        ...
        m_request = request;
        m_pending = true;
        return scheduled ? UdpRebindQueueResult::Scheduled : UdpRebindQueueResult::Replaced;
    }
    ...
    bool IsPending(const UdpRebindRequest& request) const {
        return m_pending && m_request.peer_index == request.peer_index && m_request.activation_generation == request.activation_generation;
    }
...
class UdpBinding {
  public:
    struct Snapshot {
        ...
        bool IsOpen() const {
            return socket != wgnx::platform::InvalidSocket;
        }
        bool Matches(SocketGeneration expected_generation, wgnx::platform::socket_handle expected_socket) const {
    ...
    bool HasEndpoint() const {
        return m_has_endpoint;
    }
    bool IsOpen() const {
        return m_socket != wgnx::platform::InvalidSocket;
    }
    ...
    const wgnx::platform::endpoint& Endpoint() const {
        return m_endpoint;
    }
    const char* EndpointText() const {
        return m_endpoint_text;
    }
    wgnx::platform::socket_handle Socket() const {
        return m_socket;
    }
    SocketGeneration Generation() const {
        return m_generation;
    }
...
static_assert(sizeof(UdpRebindQueue) <= wgnx::resource_budget::MaximumUdpRebindQueueBytes);
