...
bool RuntimeCoordinator::IsActiveIdentity(const PeerIdentity& identity) const {
    ...
    return peer != nullptr && m_peers.ActivePeerIndex() == static_cast<std::int32_t>(identity.peer_index.Value()) &&
           peer->IsCurrentActivation(identity.activation_generation);
}

bool RuntimeCoordinator::IsActivePathIdentity(const PeerIdentity& identity, PathRequestGeneration path_generation) const {
    const auto* peer = PeerAt(identity.peer_index);
    return IsActiveIdentity(identity) && peer != nullptr && peer->IsCurrentPathRequest(identity.activation_generation, path_generation);
}
...
bool RuntimeCoordinator::IsActiveEstablishedIdentity(const PeerIdentity& identity) const {
    const auto* peer = PeerAt(identity.peer_index);
    return IsActiveIdentity(identity) && peer->Lifecycle().state == wgnx::PeerRuntimeState::Active;
}
...
bool RuntimeCoordinator::SnapshotReceiveRuntime(ReceiveRuntimeSnapshot& out) const {
    ...
    const auto peer_index = PeerIndex{static_cast<std::uint32_t>(m_peers.ActivePeerIndex())};
...
bool RuntimeCoordinator::SnapshotDebugPeer(DebugPeerSnapshot& out) const {
    ...
    const auto peer_index = PeerIndex{static_cast<std::uint32_t>(m_peers.ActivePeerIndex())};
    const auto* peer = PeerAt(peer_index);
    if (peer == nullptr || peer->Lifecycle().state != wgnx::PeerRuntimeState::Active || !peer->BindingSnapshot().IsOpen() ||
        !peer->CanSendTransportNow()) {
        return false;
    }
    out.peer = {
        .peer_index = peer_index,
...
bool RuntimeCoordinator::HasPendingDatagram(const PeerIdentity& identity, DatagramGeneration datagram_generation) const {
    const auto* peer = PeerAt(identity.peer_index);
