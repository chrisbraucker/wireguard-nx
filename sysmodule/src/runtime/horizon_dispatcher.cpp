...
void HorizonDispatcher::Initialize(const HorizonDispatcherCallbacks &callbacks) {
    ...
    m_resolve_queue = wgnx::platform::alloc_ordered_workqueue(
        "wgnx-resolve",
        wgnx::resource_budget::ResolveWorkSlots);
    ...
    m_timer_queue = wgnx::platform::alloc_ordered_workqueue(
        "wgnx-timer-act",
...
wgnx::platform::workqueue_struct *HorizonDispatcher::QueueForLane(
    DispatcherWorkLane lane) const {
    switch (lane) {
...
wgnx::platform::workqueue_statistics HorizonDispatcher::Statistics(
    DispatcherWorkLane lane) const {
