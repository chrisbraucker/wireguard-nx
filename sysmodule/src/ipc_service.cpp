...
ams::Result ControlService::ListPeers(
    ams::sf::Out<u32> out_count,
    const ams::sf::OutArray<wgnx::PeerInfo> &out) {
...
ams::Result ControlService::TriggerDebugPayload(const wgnx::DebugTriggerRequest &request) {
    R_RETURN(runtime::TriggerDebugPayload(
        static_cast<wgnx::DebugTriggerAction>(request.action)));
}
...
ams::Result ControlService::SubmitInnerIpv4Packet(
    ...
    out.SetValue(runtime::SubmitInnerIpv4Packet(
...
ams::Result ControlService::ReceiveInnerIpv4Packet(
    ...
    const ams::sf::ClientProcessId &client_pid) {
    out.SetValue(runtime::ReceiveInnerIpv4Packet(
        {static_cast<std::uint8_t *>(packet.GetPointer()), packet.GetSize()},
        client_pid.GetValue().value));
    R_SUCCEED();
}
