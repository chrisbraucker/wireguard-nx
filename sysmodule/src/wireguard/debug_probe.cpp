...
DebugProbeReplyValidation ValidateDebugIcmpEchoReply(
    ...
    if (ihl < DebugProbeIpv4HeaderSize || payload.size() < ihl ||
        ComputeInternetChecksum(payload.data(), ihl) != 0) {
    ...
    if (icmp_size < (DebugProbeIcmpHeaderSize + DebugProbeIcmpPayloadSize) ||
