...
public:
    ...
    bool Read(std::array<std::uint8_t, Size> &value) {
        if (!CanRead(Size)) {
            return false;
        }
...
public:
    ...
    template<std::size_t Size>
    bool Write(const std::array<std::uint8_t, Size> &value) {
        if (!CanWrite(Size)) {
            return false;
        }
...
ParseResult ValidatePacket(
    ...
    if ((exact_size && packet.size() != required_size) ||
        (!exact_size && packet.size() < required_size)) {
...
const char *GetMessageTypeName(MessageType type) {
    switch (type) {
        ...
        case MessageType::HandshakeInitiation: return "handshake_initiation";
        case MessageType::HandshakeResponse: return "handshake_response";
        case MessageType::CookieReply: return "cookie_reply";
        case MessageType::TransportData: return "transport_data";
    }
    return "unknown";
}
...
const char *GetParseErrorName(ParseError error) {
    switch (error) {
        ...
        case ParseError::MissingType: return "missing_type";
        case ParseError::UnknownType: return "unknown_type";
        case ParseError::InvalidLength: return "invalid_length";
        case ParseError::InvalidArgument: return "invalid_argument";
        case ParseError::InsufficientCapacity: return "insufficient_capacity";
    }
    return "unknown";
}
...
ParseResult ParseHandshakeInitiation(
    ...
    const bool parsed = reader.ReadLe32(out_message.type) &&
                        reader.ReadLe32(out_message.sender_index) &&
                        reader.Read(out_message.unencrypted_ephemeral) &&
                        reader.Read(out_message.encrypted_static) &&
                        reader.Read(out_message.encrypted_timestamp) &&
                        reader.Read(out_message.macs.mac1) &&
...
ParseResult ParseHandshakeResponse(
    ...
    const bool parsed = reader.ReadLe32(out_message.type) &&
                        reader.ReadLe32(out_message.sender_index) &&
                        reader.ReadLe32(out_message.receiver_index) &&
                        reader.Read(out_message.unencrypted_ephemeral) &&
                        reader.Read(out_message.encrypted_nothing) &&
                        reader.Read(out_message.macs.mac1) &&
                        reader.Read(out_message.macs.mac2);
    return parsed ? result : MakeFailure(ParseError::InvalidLength, result.type);
}
...
ParseResult ParseHandshakeCookie(
    ...
    const bool parsed = reader.ReadLe32(out_message.type) &&
                        reader.ReadLe32(out_message.receiver_index) &&
                        reader.Read(out_message.nonce) &&
                        reader.Read(out_message.encrypted_cookie);
    return parsed ? result : MakeFailure(ParseError::InvalidLength, result.type);
}
...
ParseResult ParseTransportDataHeader(
    ...
    const bool parsed = reader.ReadLe32(out_message.type) &&
                        reader.ReadLe32(out_message.receiver_index) &&
                        reader.ReadLe64(out_message.counter);
    return parsed ? result : MakeFailure(ParseError::InvalidLength, result.type);
}
...
ParseError SerializeHandshakeInitiation(
    ...
    const ParseError error = ValidateSerializeTarget(output, HandshakeInitiationSize, GetMessageType(message.type), MessageType::HandshakeInitiation);
    if (error != ParseError::None) {
        return error;
    }
    PacketWriter writer(output.first(HandshakeInitiationSize));
    return writer.WriteLe32(message.type) &&
                   writer.WriteLe32(message.sender_index) &&
                   writer.Write(message.unencrypted_ephemeral) &&
                   writer.Write(message.encrypted_static) &&
                   writer.Write(message.encrypted_timestamp) &&
                   writer.Write(message.macs.mac1) &&
...
ParseError SerializeHandshakeResponse(
    ...
    const ParseError error = ValidateSerializeTarget(output, HandshakeResponseSize, GetMessageType(message.type), MessageType::HandshakeResponse);
    if (error != ParseError::None) {
        return error;
    }
    ...
    return writer.WriteLe32(message.type) &&
                   writer.WriteLe32(message.sender_index) &&
                   writer.WriteLe32(message.receiver_index) &&
                   writer.Write(message.unencrypted_ephemeral) &&
                   writer.Write(message.encrypted_nothing) &&
                   writer.Write(message.macs.mac1) &&
                   writer.Write(message.macs.mac2)
               ? ParseError::None
               : ParseError::InsufficientCapacity;
...
ParseError SerializeTransportDataHeader(
    ...
    const ParseError error = ValidateSerializeTarget(output, TransportDataHeaderSize, GetMessageType(message.type), MessageType::TransportData);
    if (error != ParseError::None) {
        return error;
    }
    ...
    return writer.WriteLe32(message.type) &&
                   writer.WriteLe32(message.receiver_index) &&
                   writer.WriteLe64(message.counter)
               ? ParseError::None
               : ParseError::InsufficientCapacity;
}
