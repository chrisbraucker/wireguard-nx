...
alignas(ams::os::ThreadStackAlignment)
constinit std::array<
    std::byte,
    wgnx::resource_budget::NifmPathThreadStackBytes> g_network_path_stack{};
...
Result SetRequestPersistent(NifmRequest *request) {
    ...
    return serviceDispatchIn(
        std::addressof(request->s),
        12,
        PersistentRequestEnabled);
}

Result GetRequestStateDirect(
    NifmRequest *request,
    NifmRequestState *out_state) {
...
bool NetworkPathService::Start(
    ...
    std::memcpy(
        m_request_storage.data(),
        std::addressof(zeroed_request),
        sizeof(zeroed_request));
    ...
    const Result requirement_result = SetRequestRequirementPreset(
        request,
        SystemRequestRequirementPreset);
...
void NetworkPathService::Stop(std::uint32_t request_generation) {
    {
        std::scoped_lock lock(m_mutex);
        if (!m_request_open || request_generation == 0 ||
            request_generation != m_request_generation) {
            return;
        }
...
void NetworkPathService::PublishCurrentObservation() {
    ...
    void *callback_context = nullptr;
    {
        ...
        const Result state_result = GetRequestStateDirect(
            Request(m_request_storage),
            std::addressof(state));
        observation.raw_state = MapRawState(state);
        observation.state_result = state_result;
        observation.operation_result = R_SUCCEEDED(state_result)
            ? GetRequestResultDirect(Request(m_request_storage))
            : state_result;
        observation.request_generation = m_request_generation;
        observation.availability = wgnx::platform::classify_network_path_state(
            observation.raw_state,
            observation.state_result);
        m_available = observation.availability ==
